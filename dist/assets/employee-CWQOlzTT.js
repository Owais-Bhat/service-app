  ...
  `}var C=!0;async function w(){try{let e=window.location.hostname!==`localhost`&&window.location.hostname!==`127.0.0.1`?`/api`:`http://localhost:5000/api`;C=(await(await fetch(`${e}/settings/device-tracking`)).json()).enabled!==!1}catch{C=!0}return C}function T(e){return[`resolved`,`case_closed`,`foc`].includes(P(e))}async function E(e,t){let n=window.location.hostname!==`localhost`&&window.location.hostname!==`127.0.0.1`?`/api`:`http://localhost:5000/api`,r=await fetch(`${n}/device-tracking/toggle`,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${localStorage.getItem(`auth_token`)}`},body:JSON.stringify({inquiry_id:e,enabled:t})});if(!r.ok){let e=await r.json().catch(()=>({}));throw Error(e.error||`Could not update device flag`)}return r.json()}function ee({maxWaitMs:e=8e3}={}){return new Promise((t,n)=>{if(!navigator.geolocation)return n(Error(`Geolocation not supported`));let r=!1,i=()=>{navigator.geolocation.getCurrentPosition(e=>{r||(r=!0,t(e))},e=>{r||(r=!0,n(e))},{enableHighAccuracy:!1,timeout:6e3,maximumAge:6e4})},a=setTimeout(i,e);navigator.geolocation.getCurrentPosition(e=>{clearTimeout(a),r||(r=!0,t(e))},()=>{clearTimeout(a),r||i()},{enableHighAccuracy:!0,timeout:e,maximumAge:0})})}async function D(e,t){return(await(await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${e}&lon=${t}&zoom=18&addressdetails=1`)).json()).display_name||``}function O(e,t){return`https://www.google.com/maps?q=${encodeURIComponent(`${e},${t}`)}`}function te(e){return e?.customer_lat!=null&&e?.customer_lng!=null?O(e.customer_lat,e.customer_lng):`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(e?.location||``)}`}function k(e,t=`Loading...`){if(!e)return()=>{};let n=e.innerHTML;return e.disabled=!0,e.classList.add(`is-loading`),e.innerHTML=`<span class="btn-spinner"></span><span>${t}</span>`,()=>{e.disabled=!1,e.classList.remove(`is-loading`),e.innerHTML=n}}async function ne(e,t,r,i,a){let o=k(e,`Loading`);try{await Fe(t,r,i,a)}catch(e){console.error(e),n(`Could not open service manager`,`error`)}finally{o()}}var A={none:`Not started`,awaiting_parts:`⏳ Awaiting Parts`,repair_progress:`🔧 In Repair`,ready_return:`📦 Ready to Return`,returned:`✅ Returned`,taken:`📸 Taken`};async function j(e,t,i={}){let a=e.querySelector(`#devices-in-service-card`),o=e.querySelector(`#devices-in-service-list`);if(!a||!o)return;if(!await w()){a.dataset.hasDevices=`0`,i.deferShow||(a.style.display=`none`);return}let{data:s}=await y(t),c=(s||[]).filter(e=>Number(e.device_service_enabled)===1);if(c.length===0){a.dataset.hasDevices=`0`,i.deferShow||(a.style.display=`none`);return}o.innerHTML=c.map(e=>{let t=e.follow_up_status&&e.follow_up_status!==`none`?e.follow_up_status:e.device_taken_logs?`taken`:`none`;return`
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px;border:1px solid var(--border);border-radius:12px;margin-bottom:8px;">
        <div style="min-width:0;">
          <div style="font-weight:700;font-size:0.9rem;color:var(--primary);">${G(e.ticket_no||`—`)}</div>
          <div style="font-size:0.82rem;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${G(e.full_name||`Client`)} · ${G(e.service_item||`Service`)}</div>
          <span class="badge" style="margin-top:4px;display:inline-block;">${A[t]||t}</span>
        </div>
        <button class="btn btn-secondary btn-sm dev-manage-btn" data-id="${e.id}" data-status="${K(e.status||`in_progress`)}">Manage</button>
      </div>`}).join(``),a.dataset.hasDevices=`1`,i.deferShow||(a.style.display=`block`);try{let e=c.filter(e=>(e.follow_up_status||`none`)!==`returned`),i=`dev_reminder_${t}_${new Date().toISOString().slice(0,10)}`;e.length&&!localStorage.getItem(i)&&(localStorage.setItem(i,`1`),r({title:`Device follow-up reminder`,body:`You have ${e.length} device(s) in service. Please update their status today.`,type:`alert`,tag:`device-followup-daily`}),n(`Reminder: update follow-up for ${e.length} device(s) in service`,`info`))}catch{}o.querySelectorAll(`.dev-manage-btn`).forEach(n=>{n.onclick=()=>ne(n,null,n.dataset.id,n.dataset.status,()=>{document.querySelector(`#task-list`)?Pe(e):j(e,t)})})}async function M(e){let{data:{user:t}}=await f.auth.getUser();if(!t){e.innerHTML=`<p>Please sign in.</p>`;return}if(!await w()){e.innerHTML=`
    ...
    <div id="fu-list"><div style="text-align:center;padding:30px;color:var(--text-dim);">Loading…</div></div>`;let r=e.querySelector(`#fu-list`),{data:i}=await y(t.id),a=(i||[]).filter(e=>Number(e.device_service_enabled)===1||e.device_taken_logs);if(a.length===0){r.innerHTML=`<div class="card"><div class="card-body" style="text-align:center;padding:40px;color:var(--text-dim)">No devices in service right now. Take a device from a ticket's <b>Device Service</b> tab to start tracking.</div></div>`;return}let o=await Promise.all(a.map(e=>b(e.id)));r.innerHTML=a.map((e,t)=>{let n=o[t]?.data?.device_follow_up_logs||[];return`
      <div class="card" style="margin-bottom:14px;" data-inq="${K(e.id)}">
        <div class="card-header"><span class="card-title">${G(e.ticket_no||`—`)} · ${G(e.full_name||`Client`)}</span></div>
        <div class="card-body">
          <div style="font-size:0.85rem;color:var(--text-soft);margin-bottom:12px;">${G(e.service_item||`Service`)}</div>
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:8px 18px;background:var(--bg-soft);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:14px;font-size:0.84rem;">
            ${[[`Customer`,e.full_name],[`Phone`,e.phone],[`Company`,e.company_name],[`Address`,e.address],[`Device`,e.device_type],[`Serial No`,e.device_serial_no],[`Preferred Time`,e.preferred_time],[`Bill No`,e.bill_no]].filter(([,e])=>e).map(([e,t])=>`
              <div><div style="color:var(--text-dim);font-weight:600;font-size:0.72rem;text-transform:uppercase;letter-spacing:.04em;">${e}</div><div>${G(t)}</div></div>
            `).join(``)}
          </div>
    ...
    </div>`,d=[u(`Services subtotal`,n(e.servicesSubtotal)),Number(e.extra)>0?u(`Extra charges`,n(e.extra)):``,u(`Platform fee`,n(e.platform)),u(`Transport`,n(e.transport)),Number(e.discount)>0?u(r(e.discountLabel||`Discount`),`-${n(e.discount)}`,{color:`#059669`}):``,u(`Taxable`,n(e.taxable),{bold:!0,border:`border-top:1px solid #eee !important; padding-top:5px !important;`}),u(`GST (18%)`,n(e.gst)),u(`Total`,n(e.total),{bold:!0,total:!0,border:`border-top:2px solid #10B981 !important; margin-top:2px !important;`})].join(``);return`
...
</html>`),n.document.close()}function ae(e){let t=e.querySelector(`#bill-preview-stage`),n=e.querySelector(`#bill-preview-container`),r=e.querySelector(`.modal-body`);if(!t||!n||!r)return;n.style.transform=`scale(1)`,t.style.height=`auto`;let i=Math.max(260,r.clientWidth-16),a=Math.min(1,i/794);n.style.transform=`scale(${a})`,t.style.height=`${n.offsetHeight*a}px`}function B(e){return new Promise((t,n)=>{let r=new FileReader;r.onload=()=>{let e=String(r.result||``);t(e.includes(`,`)?e.slice(e.indexOf(`,`)+1):e)},r.onerror=()=>n(r.error||Error(`Could not read PDF blob`)),r.readAsDataURL(e)})}async function oe(e,t,n){let r=await B(e),i=localStorage.getItem(`auth_token`),a=await fetch(`/api/bills/upload`,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${i}`},body:JSON.stringify({dataBase64:r,filename:t,inquiry_id:n||null})});if(!a.ok){let e=await a.json().catch(()=>({}));throw Error(e.error||`Upload failed`)}let{url:o}=await a.json();return o}async function se(e,t){let n=localStorage.getItem(`auth_token`),r=`Invoice-${e.customer?.ticket_no||`service`}.pdf`,i=await fetch(`/api/bills/generate`,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${n}`},body:JSON.stringify({billData:e,inquiry_id:t||null,filename:r})});if(!i.ok){let e=await i.json().catch(()=>({}));throw Error(e.error||`Server PDF generation failed`)}let{url:a}=await i.json();return a}async function ce(e,{inquiryId:t=null,existingUrl:n=null}={}){if(n)return n;try{return await se(e,t)}catch(n){console.warn(`[bill] server PDF unavailable, using browser render:`,n.message);let r=R(e),i=`Invoice-${e.customer?.ticket_no||`service`}.pdf`,{blob:a}=await z(r,i);return oe(a,i,t)}}function le(e,t){let n=e=>`₹${Math.round(Number(e)||0).toLocaleString(`en-IN`)}`,r=Array.isArray(e.services)?e.services:[],i=[`Hi ${e.customer?.name||`Customer`}!`,`Your service invoice from *${N.name}* is ready.`,`Ticket: *${e.customer?.ticket_no||`-`}*`,`Device: *${e.customer?.device_type||`General Service`}*`,``,`*Bill Breakdown:*`];return r.length?r.forEach((e,t)=>i.push(`${t+1}. ${e.name}: *${n(e.cost)}*`)):i.push(`- Services Subtotal: *${n(e.servicesSubtotal)}*`),e.extra>0&&i.push(`- Additional Charges: *${n(e.extra)}*${e.extraReason?` (${e.extraReason})`:``}`),i.push(`- Platform Fee: *${n(e.platform)}*`),e.km>0&&i.push(`- Transport (${e.km} km): *${n(e.transport)}*`),e.discount>0&&i.push(`- Discount: *-${n(e.discount)}*${e.discountLabel?` (${e.discountLabel})`:``}`),i.push(`- GST (18%): *${n(e.gst)}*`,`------------------------------`,`*Total Payable: ${n(e.total)}*`,`------------------------------`,`Payment Status: *${String(e.paymentStatus||`unpaid`).toUpperCase()}*`),t&&i.push(``,`View / download your invoice (PDF):`,t),e.paymentStatus!==`paid`&&e.paymentLink&&i.push(``,`Pay here:`,e.paymentLink),i.push(``,`- ${N.name}`),i.join(`
`)}function ue(e,t={}){let{onSent:r,allowShare:i=!0,title:a=`Service Invoice Preview`,inquiryId:o=null,existingPdfUrl:s=null}=t,c=R(e),l=document.createElement(`div`);l.className=`modal-overlay premium-bill-modal`,l.innerHTML=`
    ...
    </div>`,document.body.appendChild(l),requestAnimationFrame(()=>ae(l));let d=()=>ae(l);window.addEventListener(`resize`,d);let f=e=>{e.key===`Escape`&&p()},p=()=>{window.removeEventListener(`resize`,d),window.removeEventListener(`keydown`,f),l.remove()};window.addEventListener(`keydown`,f),l.querySelector(`#pb-close`).onclick=p,l.querySelector(`#pb-cancel`).onclick=p,l.addEventListener(`click`,e=>{e.target===l&&p()});let m=`Invoice-${e.customer?.ticket_no||`service`}.pdf`;l.querySelector(`#pb-print`).onclick=()=>{try{ie(c,m)}catch(e){console.error(e),n(e.message||`Could not open print window`,`error`)}},i&&(l.querySelector(`#pb-whatsapp`).onclick=async()=>{let t=l.querySelector(`#pb-whatsapp`);t.disabled=!0;let i=t.innerHTML,a=(e.customer?.phone||``).replace(/\D/g,``);if(!a){n(`Client phone number is missing on this inquiry`,`error`),t.disabled=!1,t.innerHTML=i;return}try{t.innerHTML=`<span>... preparing PDF</span>`;let i=await ce(e,{inquiryId:o,existingUrl:s}),c=le(e,i),l=`https://wa.me/${a}?text=${encodeURIComponent(c)}`;if(window.open(l,`_blank`),n(`WhatsApp opened with PDF link - send the message to the client.`,`success`),typeof r==`function`)try{await r(i)}catch{}}catch(e){console.error(e),n(e.message||`Could not send bill`,`error`)}finally{t.disabled=!1,t.innerHTML=i}})}function de(e=new Date){return e.toLocaleDateString(`en-CA`).slice(0,7)}function fe(e){if(!e)return``;if(e instanceof Date)return Number.isNaN(e.getTime())?``:e.toLocaleDateString(`en-CA`);let t=String(e).trim(),n=/^(\d{4}-\d{2}-\d{2})/.exec(t);if(n)return n[1];let r=new Date(t.replace(` `,`T`));return Number.isNaN(r.getTime())?``:r.toLocaleDateString(`en-CA`)}function V(e){return fe(e?.date||e?.clock_in)}function pe(e,t){if(!e||!t)return 0;let n=fe(e),r=fe(t);if(!n||!r)return 0;let i=new Date(`${n}T00:00:00`),a=new Date(`${r}T00:00:00`);return Number.isNaN(i.getTime())||Number.isNaN(a.getTime())||a<i?0:Math.floor((a-i)/864e5)+1}function me(e,t){if(!e||!t)return null;let n=new Date(t)-new Date(e);return!Number.isFinite(n)||n<0?null:`${Math.floor(n/36e5)}h ${Math.floor(n%36e5/6e4)}m`}var he=`18:00`,ge=4;function H(e=he){let t=/^(\d{1,2}):(\d{2})$/.exec(String(e||``).trim());if(!t)return{hour:18,minute:0,label:`18:00`};let n=Math.min(23,Math.max(0,Number(t[1]))),r=Math.min(59,Math.max(0,Number(t[2])));return{hour:n,minute:r,label:`${String(n).padStart(2,`0`)}:${String(r).padStart(2,`0`)}`}}async function _e(){let e=window.location.hostname!==`localhost`&&window.location.hostname!==`127.0.0.1`?`/api`:`http://localhost:5000/api`;try{let t=await fetch(`${e}/settings/attendance`,{headers:{Authorization:`Bearer ${localStorage.getItem(`auth_token`)||``}`}});t.ok&&(he=H((await t.json()).autoClockOutTime).label)}catch(e){console.warn(`[employee] could not load clock-out settings`,e)}return H()}function ve(e=new Date){let{hour:t,minute:n}=H(),r=new Date(e);return r.setHours(t,n,0,0),e>=r}var ye=8,be=30;function xe(e=new Date){let t=new Date(e);return t.setHours(ye,be,0,0),e<t}function U(e,t=new Date().toLocaleDateString(`en-CA`)){return!e?.clock_in||e?.clock_out?!1:V(e)!==t||ve()}function Se(e=[],t=[],n=new Date().toLocaleDateString(`en-CA`)){let r=new Set((t||[]).map(e=>fe(e.date||e.created_at)).filter(Boolean));return(e||[]).filter(e=>{let t=V(e);return!e?.clock_in||!t||r.has(t)?!1:t!==n||ve()})}function Ce(e){return`\u20B9${Math.round(Number(e)||0).toLocaleString(`en-IN`)}`}function W(e,t){let n=e?.created_at||e?.inquiries?.[0]?.created_at||0,r=t?.created_at||t?.inquiries?.[0]?.created_at||0;return new Date(r)-new Date(n)}function we(e,t=new Date){if(!e)return`-`;let n=new Date(e),r=new Date(t);if(Number.isNaN(n.getTime())||Number.isNaN(r.getTime()))return`-`;let i=Math.max(0,r.getTime()-n.getTime()),a=Math.floor(i/864e5),o=Math.floor(i%864e5/36e5),s=Math.floor(i%36e5/6e4);return a?`${a}d ${o}h`:o?`${o}h ${s}m`:`${s}m`}function G(e){return String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e])}function K(e){return G(e)}function Te(e){let t=Math.round(Number(e)||0);return Array.from({length:5},(e,n)=>`<span style="color:${n<t?`var(--warning)`:`var(--border)`};display:inline-flex;width:14px;height:14px">${n<t?u.star:u.starOutline}</span>`).join(``)}async function Ee(){let{data:{user:e}}=await f.auth.getUser();if(!e)return{user:null};let[{data:t},{data:n},{data:r},{data:i}]=await Promise.all([f.from(`profiles`).select(`*`).eq(`id`,e.id).maybeSingle(),f.from(`attendance`).select(`*`).eq(`user_id`,e.id).order(`date`,{ascending:!1}),f.from(`leave_requests`).select(`*`).eq(`employee_id`,e.id).order(`created_at`,{ascending:!1}),f.from(`eod_reports`).select(`*`).eq(`employee_id`,e.id).order(`date`,{ascending:!1})]);return{user:e,profile:t||e,attendance:n||[],leaves:r||[],reports:i||[]}}async function De(e){let{read:t,utils:n}=await p(async()=>{let{read:e,utils:t}=await import(`https://cdn.sheetjs.com/xlsx-0.18.5/package/xlsx.mjs`);return{read:e,utils:t}},[]),r=t(await e.arrayBuffer()),i=r.Sheets[r.SheetNames[0]];if(!i)return[];let a=n.sheet_to_json(i,{header:1});return a.length>1?a.slice(1):[]}async function Oe(e){let t=0,n=0,r=[],i=[];for(let t of e){let[e,r,a,o]=Array.isArray(t)?t:[t.category,t.sub_category,t.sub_sub_category,t.cost];if(!e||!a){n++;continue}let s=parseFloat(o)||0;if(s<0){n++;continue}i.push({id:crypto.randomUUID?.()||`svc-${Date.now()}-${Math.random()}`,category:String(e).trim(),sub_category:r?String(r).trim():null,sub_sub_category:String(a).trim(),name:String(a).trim(),cost:s})}for(let e=0;e<i.length;e+=10){let a=i.slice(e,e+10),o=0;for(;o<3;){let{error:e}=await f.from(`service_pricing`).insert(a);if(e?.status===429){o++,await new Promise(e=>setTimeout(e,1e3*2**o));continue}e?(r.push(e.message),n+=a.length):t+=a.length;break}o===3&&(r.push(`Rate limited - batch skipped`),n+=a.length)}return{inserted:t,skipped:n,errors:r}}function ke(){let e=[[`Main Category`,`Sub Category`,`Sub-Sub Category`,`Price`],[`Network Installation`,`Setup`,`Basic Setup`,`500`],[`Network Installation`,`Setup`,`Advanced Setup`,`1000`],[`Repair`,`Hardware`,`Cable Replacement`,`300`]].map(e=>e.map(e=>`"${e}"`).join(`,`)).join(`
`),t=new Blob([e],{type:`text/csv`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=`service-pricing-template.csv`,r.click(),URL.revokeObjectURL(n)}async function q(e){a(e);let{data:{user:t}}=await f.auth.getUser();if(!t){e.innerHTML=`<p>Please sign in.</p>`;return}let s=new Date().toLocaleDateString(`en-CA`),c=await _e(),d,p=[],m=[],h,g,_=[],v=[],y=[],b=[],x=[];try{let e=await Promise.all([f.from(`attendance`).select(`*`).eq(`user_id`,t.id).eq(`date`,s).maybeSingle(),f.from(`tickets`).select(`*, inquiries(*)`).eq(`assigned_to`,t.id).order(`created_at`,{ascending:!1}),f.from(`eod_reports`).select(`*`).eq(`employee_id`,t.id).eq(`date`,s).maybeSingle(),f.from(`inquiries`).select(`*`).eq(`assigned_employee_id`,t.id).in(`assignment_status`,[`pending`,`accepted`]).order(`created_at`,{ascending:!1}),f.from(`attendance`).select(`*`).eq(`user_id`,t.id).order(`date`,{ascending:!1}),f.from(`notices`).select(`*`).eq(`active`,1).order(`created_at`,{ascending:!1}),f.from(`eod_reports`).select(`*`).eq(`employee_id`,t.id).order(`date`,{ascending:!1}),f.from(`inquiries`).select(`feedback_rating,employee_rating,feedback_employee_id,assigned_employee_id,feedback_at,updated_at`),f.from(`profiles`).select(`id,full_name,role`)]);d=e[0].data,h=e[1].data,g=e[2].data,b=(e[7]?.data||[]).filter(e=>e.feedback_rating!=null),x=e[8]?.data||[],p=e[4].data||[],y=(e[5].data||[]).filter(e=>!e.expires_at||new Date(e.expires_at)>=new Date).slice(0,4),m=e[6].data||[];let n=e[3].data||[],r=new Map;n.forEach(e=>{e.ticket_id&&r.set(String(e.ticket_id),e)});let i=(h||[]).filter(e=>!(e.inquiries&&e.inquiries.length)&&!r.has(String(e.id))).map(e=>e.id).filter(Boolean);if(i.length){let{data:e}=await f.from(`inquiries`).select(`*`).in(`ticket_id`,i);(e||[]).forEach(e=>{e.ticket_id&&r.set(String(e.ticket_id),e)})}h=(h||[]).map(e=>{if(e.inquiries&&e.inquiries.length)return e;let t=r.get(String(e.id));return t?{...e,inquiries:[t]}:e});let a=new Set((h||[]).map(e=>e.inquiries?.[0]?.id).filter(Boolean));_=n.filter(e=>e.assignment_status===`pending`).sort(W),v=n.filter(e=>e.assignment_status===`accepted`&&!a.has(e.id)&&![`resolved`,`closed`,`case_closed`,`paid`,`foc`,`issue_not_resolved`].includes(e.status)).sort(W).map(e=>({...e,_company:e.company_name||null}))}catch(t){e.innerHTML=`<div class="card"><div class="card-body" style="text-align:center;padding:40px;"><h3 style="color:var(--danger);display:inline-flex;align-items:center;gap:8px;">${u.alert}<span>Error</span></h3><p>${t.message}</p></div></div>`;return}let S=h||[],C=new Set([`resolved`,`case_closed`,`foc`,`paid`]),w=S.filter(e=>{let t=P(e.status);if(C.has(t))return!1;let n=e.inquiries?.[0];return n?n.assignment_status===`accepted`&&!C.has(P(n.status)):t===`assigned`||t===`in_progress`||t===`open`}),T=!!d?.clock_in,E=!!d?.clock_out,O=ve(),k=T&&!E,A=Se(p,m,s),M=A.length>=ge,N=[...w,...v].sort(W),F=N.filter(e=>P((e.inquiries?.[0]||e).status||e.status)===`in_progress`).length,I=N.length?Math.round(F/N.length*100):0,L=N.filter(e=>Number((e.inquiries?.[0]||e)?.reopened)===1),R=new Map((x||[]).filter(e=>e.role===`employee`).map(e=>[e.id,e])),z=de(),ie=(b||[]).filter(e=>String(e.feedback_at||e.updated_at||``).startsWith(z)),ae=new Map;ie.forEach(e=>{let t=e.feedback_employee_id||e.assigned_employee_id;if(!t||!R.has(t))return;let n=Number(e.employee_rating||e.feedback_rating||0);if(!n)return;ae.has(t)||ae.set(t,{total:0,count:0});let r=ae.get(t);r.total+=n,r.count+=1});let B=[...ae.entries()].map(([e,t])=>({id:e,name:R.get(e)?.full_name||`Employee`,avg:t.total/t.count,count:t.count})).sort((e,t)=>t.avg-e.avg||t.count-e.count).map((e,n)=>({...e,rank:n+1,you:e.id===t.id})),oe=B.slice(0,3),se=B.find(e=>e.you);se&&!oe.some(e=>e.you)&&oe.push(se),e.innerHTML=`
    ...
    <div class="card emp-clock-row" style="margin-bottom:18px;">
      <div class="emp-clock-left">
        ...
        ${d?.location?`<p class="attendance-location" style="margin-top:8px;justify-content:flex-start;">${u.pin}<span>${G(d.location)}</span></p>`:``}
      </div>
      <div class="emp-clock-right">
        ${T||E||A.length>0?`
          <button class="btn btn-sm emp-eod-btn ${g?`emp-eod-ok`:A.length?M?`emp-eod-err`:`emp-eod-warn`:`emp-eod-ok`}" id="btn-eod-status">
            ${g?u.check:A.length?u.alert:u.clipboard}
            <span>${g?`EOD Submitted`:A.length?M?`Restricted`:`EOD Warning`:`EOD Status`}</span>
          </button>
        `:``}
        ${xe()&&!T?`<p class="attendance-lock-note" style="text-align:center;justify-content:center;margin-bottom:8px;">${u.clock}<span>Clock-in opens at 8:30 AM</span></p>`:``}
        ...
        <button class="btn ${k?`btn-danger`:`btn-primary`} emp-clock-toggle" id="btn-clock-toggle" ${E||!k&&(M||O||xe())?`disabled`:``}>
          ${k?u.pause:u.play}
    ...
    <div class="card" style="margin-bottom:18px;">
      ...
      ${y.length===0?`<p style="margin:10px 18px 14px;font-size:0.87rem;color:var(--text-dim);">No active notices right now.</p>`:`<div class="emp-notices-list">${y.map((e,t)=>`<div class="emp-notice-item" data-notice-idx="${t}" role="button" tabindex="0"><div class="emp-notice-ico">${u.bell}</div><div style="flex:1;min-width:0;"><div class="emp-notice-title">${G(e.title||`Notice`)}</div><div class="emp-notice-date">${o(e.created_at)}</div></div><span style="color:var(--text-dim);flex-shrink:0;">${u.arrowRight}</span></div>`).join(``)}</div>`}
    </div>
        ...
        <div class="list">
          ${_.map(e=>`
            <div class="lrow">
              ...
              <div class="lrow-main">
                <b>${G(e.full_name||`Client`)}</b>
                <span class="lsub"><em class="id-mono" style="font-style:normal">${G(e.ticket_no||`No ticket`)}</em> · ${G(e.service_item||`Service request`)} · ${l(e.created_at)}</span>
              </div>
              <button class="btn btn-primary btn-sm accept-btn" data-id="${K(e.id)}" data-ticket-id="${K(e.ticket_id||``)}">${u.check}<span>Accept</span></button>
              <button class="btn btn-secondary btn-sm decline-btn" data-id="${K(e.id)}">${u.close}<span class="hide-sm">Decline</span></button>
            </div>
          `).join(``)}
        </div>
      </div>
        ...
        ${N.length===0?`<div style="text-align:center;padding:32px 18px;color:var(--text-3,var(--text-dim));font-size:0.86rem;">No tasks assigned right now.</div>`:N.map((e,t)=>{let n=e.inquiries?.[0]||e,r=e.inquiries?e.id:n.ticket_id||``,i=P(n.status||e.status||`assigned`),a=i===`in_progress`,o=t===N.length-1,s=a?`progress`:i===`assigned`?`assigned`:`open`;return`
              <div class="route-stop">
                ...
                <div class="route-body">
                  <div class="route-name">${G(n.full_name||e.title||`Service task`)}</div>
                  <div class="route-sub"><em class="id-mono" style="font-style:normal">${G(n.ticket_no||`No ticket`)}</em> · ${G(n.service_item||e.description||`Service request`)}</div>
                  ...
                  <div class="route-actions">
                    ...
                    <button class="btn btn-secondary btn-sm task-btn" data-id="${K(r)}" data-inq-id="${K(n.id||``)}" data-status="${K(n.status||e.status||`assigned`)}">${u.edit}<span>Open</span></button>
                  </div>
                </div>
              </div>`}).join(``)}
      </div>
        ...
        <div class="list">
          ${L.map(e=>{let t=e.inquiries?.[0]||e,n=e.inquiries?e.id:t.ticket_id||``;return`
            <div class="lrow">
              ...
              <div class="lrow-main">
                <b>${G(t.full_name||e.title||`Client`)}</b>
                <span class="lsub"><em class="id-mono" style="font-style:normal">${G(t.ticket_no||`No ticket`)}</em> · ${G(t.service_item||`Service`)} · free rework</span>
              </div>
              <button class="btn btn-secondary btn-sm task-btn" data-id="${K(n)}" data-inq-id="${K(t.id||``)}" data-status="${K(t.status||e.status||`in_progress`)}">${u.edit}<span>Open</span></button>
            </div>`}).join(``)}
        </div>
      </div>
    `:``}
        ...
        <div class="list">
          ${v.map(e=>{let t=P(e.status),n=t===`in_progress`?`progress`:t===`resolved`?`resolved`:t===`assigned`?`assigned`:`open`;return`
            <div class="lrow emp-job-card" data-status="${t}">
              ...
              <button class="btn btn-primary btn-sm" onclick="window.open('${K(te(e))}')">
  ...
  `;let ce=e.querySelector(`#live-clock`),le=()=>{ce&&(ce.textContent=new Date().toLocaleTimeString())};le(),setInterval(le,1e3);let ue=e.querySelector(`#emp-company-tabs`);ue&&ue.querySelectorAll(`.sr-filter`).forEach(t=>{t.onclick=()=>{ue.querySelectorAll(`.sr-filter`).forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`);let n=t.dataset.company;e.querySelectorAll(`.emp-job-card`).forEach(e=>{e.style.display=n===`all`||e.dataset.company===n?``:`none`})}});let fe=(t,n)=>{let r=e.querySelector(t);r&&(r.onclick=n)};fe(`#btn-clock-toggle`,async()=>{if(k){if(!g){pe();return}await V();return}if(M){n(`Clock-in is restricted because you have 4 or more missed EOD reports. Contact admin.`,`error`);return}if(xe()){n(`Clock-in is not allowed before 8:00 AM.`,`error`);return}if(ve()){n(`Clock-in is closed after ${H().label}. Please contact admin.`,`error`);return}let r=e.querySelector(`#btn-clock-toggle`);r.disabled=!0,r.innerHTML=`${u.crosshair}<span>Getting location…</span>`;let i=`Unknown`,a={lat:null,lng:null,accuracy:null};try{let{latitude:e,longitude:t,accuracy:n}=(await ee()).coords;a={lat:e,lng:t,accuracy:n};try{i=await D(e,t)||`${e.toFixed(6)}, ${t.toFixed(6)}`}catch{i=`${e.toFixed(6)}, ${t.toFixed(6)}`}}catch{}let{error:o}=await f.from(`attendance`).insert({user_id:t.id,clock_in:new Date().toISOString(),date:s,location:i,latitude:a.lat,longitude:a.lng,status:`present`});o?(n(o.message,`error`),r.disabled=!1,r.innerHTML=`${u.play}<span>Clock In</span>`):(n(`Clocked in!`,`success`),q(e))});let V=async()=>{let{error:r}=await f.from(`attendance`).update({clock_out:new Date().toISOString()}).eq(`user_id`,t.id).eq(`date`,s);r?n(r.message,`error`):(n(`Clocked out!`,`success`),q(e))},pe=()=>{let e=document.createElement(`div`);e.className=`modal-overlay`,e.innerHTML=`
      ...
      <div class="modal" style="max-width:420px;">
        ...
        <div class="modal-body">
          <p style="margin:0;font-size:0.9rem;color:var(--text-soft);line-height:1.6;">${t?`Your End of Day report for today has been submitted successfully. Great work!`:`You have ${A.length} missed EOD report${A.length===1?``:`s`}. ${n?`Clock-in is restricted until you contact admin.`:`Please submit your EOD report before the day closes.`}`}</p>
          ${A.length>0?`<div style="margin-top:14px;padding:10px 12px;background:var(--bg-soft);border-radius:8px;font-size:0.83rem;color:var(--text-dim);">Missed dates: ${A.map(e=>o(e)).join(`, `)}</div>`:``}
        </div>
        ...
        <div class="modal" style="max-width:600px;">
          <div class="modal-header">
            <span class="modal-title">${u.bell}<span>${G(n.title||`Notice`)}</span></span>
          ...
          <div class="modal-body">
            ...
            <div style="line-height:1.7;white-space:pre-wrap;font-size:0.95rem;">${G(n.body||n.message||n.content||`(No details available.)`)}</div>
          </div>
        </div>`,document.body.appendChild(r);let i=()=>r.remove();r.querySelector(`#notice-close-btn`).onclick=i,r.onclick=e=>{e.target===r&&i()}}}),e.querySelector(`#btn-submit-eod`)&&fe(`#btn-submit-eod`,async()=>{let r=e.querySelector(`#eod-content`).value.trim();if(!r){n(`Please write your report`,`warning`);return}let i=e.querySelector(`#btn-submit-eod`);i.disabled=!0,i.textContent=`Submitting...`;let{error:a}=await f.from(`eod_reports`).insert({employee_id:t.id,content:r,date:s});a?(n(a.message,`error`),i.disabled=!1,i.textContent=`Submit Report →`):(n(`EOD Report submitted!`,`success`),q(e))}),document.querySelectorAll(`.clockin-gate`).forEach(e=>e.remove()),!T&&!E&&!O&&!M&&!xe()){let t=document.createElement(`div`);t.className=`modal-overlay clockin-gate`,t.innerHTML=`
      ...
      </div>`,document.body.appendChild(t),t.querySelector(`#gate-clock-in`).onclick=()=>{t.remove(),e.querySelector(`#btn-clock-toggle`)?.click()}}fe(`#btn-open-leave-modal`,()=>Ie(t.id,()=>q(e))),e.querySelectorAll(`.task-btn`).forEach(t=>{t.onclick=()=>ne(t,t.dataset.id,t.dataset.inqId,t.dataset.status,()=>q(e))}),j(e,t.id),e.querySelectorAll(`.accept-btn`).forEach(t=>{t.onclick=async()=>{let r=[f.from(`inquiries`).update({assignment_status:`accepted`,status:`in_progress`}).eq(`id`,t.dataset.id)];t.dataset.ticketId&&r.push(f.from(`tickets`).update({status:`in_progress`}).eq(`id`,t.dataset.ticketId));let i=(await Promise.all(r)).find(e=>e.error)?.error;i?n(i.message,`error`):(n(`Task accepted!`,`success`),q(e))}}),e.querySelectorAll(`.decline-btn`).forEach(t=>{t.onclick=()=>{let r=prompt(`Please provide a reason for declining:`);if(r!==null){if(!r.trim()){n(`Reason is required to decline`,`warning`);return}(async()=>{let{error:i}=await f.from(`inquiries`).update({assignment_status:`declined`,decline_reason:r.trim(),status:`open`}).eq(`id`,t.dataset.id);i?n(i.message,`error`):(n(`Task declined`,`info`),q(e))})()}}});let me=f.channel(`employee-jobs-${t.id}`).on(`postgres_changes`,{event:`*`,schema:`public`,table:`inquiries`,filter:`assigned_employee_id=eq.${t.id}`},t=>{let n=t.new;t.eventType===`INSERT`||t.eventType===`UPDATE`&&n?.assignment_status===`pending`?r({title:`🔔 New Job Assigned`,body:`${n?.full_name||`A client`} - ${n?.service_item||`new service`}`,type:`alert`,tag:`assign-${n?.id||``}`}):t.eventType===`UPDATE`&&n?.payment_status===`paid`&&r({title:`💰 Payment Received`,body:`${n?.full_name||`Client`} paid for ticket ${n?.ticket_no||``}`,type:`payment`,tag:`pay-${n?.id||``}`}),q(e)}).subscribe(),he=setInterval(()=>{document.body.contains(e)||(f.removeChannel(me),clearInterval(he))},5e3)}async function J(e){a(e);let{user:t,attendance:n,reports:r}=await Ee();if(!t){e.innerHTML=`<p>Please sign in.</p>`;return}let s=de(),c=n.filter(e=>String(e.date||``).startsWith(s)),l=new Set(c.map(e=>e.date)).size,u=c.filter(e=>e.clock_in&&e.clock_out),d=Se(n,r),f=d.length>=ge,p=u.reduce((e,t)=>e+Math.max(0,new Date(t.clock_out)-new Date(t.clock_in))/6e4,0),m=`${Math.floor(p/60)}h ${Math.round(p%60)}m`;e.innerHTML=`
    <div class="page-header">
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card"><div class="stat-value" style="color:var(--success)">${c.filter(e=>e.clock_in&&!e.clock_out&&!U(e)).length}</div><div class="stat-label">Active Sessions</div></div>
        ...
        <table>
          ...
          <tbody>
            ${n.length===0?`<tr><td colspan="5" style="text-align:center;padding:32px;color:var(--text-dim)">No attendance records yet</td></tr>`:n.map(e=>`<tr>
                ...
                <td>${e.clock_out?`<span class="badge badge-resolved">${i(e.clock_out)}</span>`:U(e)?`<span class="badge badge-open">Auto clock-out pending</span>`:`<span class="badge badge-open">Active</span>`}</td>
  ...
  `}async function Ae(e){a(e);let{user:t,leaves:n}=await Ee();if(!t){e.innerHTML=`<p>Please sign in.</p>`;return}let r=n.filter(e=>e.status===`pending`).length,i=n.filter(e=>e.status===`approved`).reduce((e,t)=>e+pe(t.start_date,t.end_date),0);e.innerHTML=`
        ...
        <table>
          ...
          <tbody>
            ${n.length===0?`<tr><td colspan="4" style="text-align:center;padding:32px;color:var(--text-dim)">No leave requests yet</td></tr>`:n.map(e=>`<tr>
                ...
                <td>${pe(e.start_date,e.end_date)}</td>
  ...
  `,e.querySelector(`#leave-new`).onclick=()=>Ie(t.id,()=>Ae(e))}async function je(e){a(e);let{user:t,reports:r}=await Ee();if(!t){e.innerHTML=`<p>Please sign in.</p>`;return}let s=new Date().toLocaleDateString(`en-CA`),c=r.find(e=>e.date===s);e.innerHTML=`
    <div class="page-header">
      <h1>EOD Reports</h1>
  ...
  `;let m={pending:i,submitted:o,all:r},h=e.querySelector(`#cash-panel`),g=e.querySelector(`#cash-filter-btn`),_=e=>{g.closest(`.df-wrap`).contains(e.target)||(h.style.display=`none`,g.classList.remove(`df-open`),document.removeEventListener(`click`,_))},v=()=>{h.style.display=`none`,g.classList.remove(`df-open`),document.removeEventListener(`click`,_)};g.onclick=e=>{e.stopPropagation(),h.style.display===`none`?(h.style.display=``,g.classList.add(`df-open`),setTimeout(()=>document.addEventListener(`click`,_),0)):v()},e.querySelector(`#cash-ok`).onclick=()=>{let t=m[e.querySelector(`#cash-status-sel`).value]||r;e.querySelector(`tbody`).innerHTML=p(t),v()},e.querySelector(`#cash-clear`).onclick=()=>{e.querySelector(`#cash-status-sel`).value=`pending`,e.querySelector(`tbody`).innerHTML=p(i),v()}}async function Y(e){a(e);let{data:{user:t}}=await f.auth.getUser();if(!t){e.innerHTML=`<p>Please sign in.</p>`;return}let n=de(),r=localStorage.getItem(`auth_token`),i=await fetch(`/api/leaderboard?month=${n}`,{headers:{Authorization:`Bearer ${r}`}}).then(e=>e.ok?e.json():null).catch(()=>null);if(!i){e.innerHTML=`<div class="card" style="padding:32px;text-align:center;color:var(--text-dim)">Could not load leaderboard. Please try again.</div>`;return}let o=i.monthly||[],s=i.allTime||[],c=o.find(e=>e.id===t.id),l=s.find(e=>e.id===t.id),u=o.findIndex(e=>e.id===t.id)+1,d=s.findIndex(e=>e.id===t.id)+1,p=[`🥇`,`🥈`,`🥉`],m=[`120px`,`88px`,`64px`],h=[1,0,2],g=e=>String(e).trim().split(/\s+/).map(e=>e[0]).slice(0,2).join(``).toUpperCase()||`?`,_=h.map(e=>o[e]||{id:null,name:`—`,count:0,avg:0,fiveStars:0,_empty:!0}),v=(e,n)=>{let r=e.id===t.id;return`
      <div class="lb-podium-slot lb-podium-slot-${n}">
        ...
        <div class="lb-podium-name">${e._empty?`<span style="color:var(--text-dim)">Available</span>`:G(e.name)+(r?` <span class="lb-you-tag">You</span>`:``)}</div>
        ...
        <div class="lb-podium-stars">${e._empty||e.count===0?`<span style="color:var(--text-dim);font-size:0.75rem;">No reviews yet</span>`:`${Te(e.avg)} <span>${e.avg.toFixed(1)}</span>`}</div>
    ...
    <div class="lb-my-rank-card" style="margin-bottom:14px;">
      <div class="lb-my-rank-left">
        ...
        <div>
          <div class="lb-my-rank-name">${G(c?.name||`You`)}</div>
    ...
    <div class="lb-my-rank-card" style="margin-bottom:14px;">
      <div class="lb-my-rank-left">
        ...
        <div>
          <div class="lb-my-rank-name">${G(l?.name||`You`)}</div>
  ...
  `}async function Ne(e){a(e);let{data:{user:t}}=await f.auth.getUser();if(!t){e.innerHTML=`<p>Please sign in.</p>`;return}try{let[{data:t},{data:r},{data:i}]=await Promise.all([f.from(`service_pricing`).select(`*`).order(`category`),f.from(`discount_presets`).select(`*`).eq(`active`,1).order(`created_at`,{ascending:!1}),f.from(`companies`).select(`*`).order(`name`)]),a={};(t||[]).forEach(e=>{let t=e.category||`Uncategorized`,n=e.sub_category&&e.sub_category.trim()||``,r=e.sub_sub_category||e.name||``;r&&(a[t]??={},a[t][n]??=[],a[t][n].push({id:e.id,leaf:r,cost:Number(e.cost)||0}))});let o=Object.keys(a).sort(),s=Array.isArray(i)?i:[],c=Array.isArray(r)?r:[];e.innerHTML=`
            ...
            <div class="estimator-grid">
              ...
              <div class="form-group">
                ...
                <select id="est-company">
                  ...
                  ${s.map(e=>`<option value="${K(e.name||``)}">${G(e.name||``)}</option>`).join(``)}
            ...
            `:`
              ...
              <div class="svc-picker-wrap">
                <select id="est-svc-main" class="svc-picker">
                  ...
                  ${o.map(e=>`<option value="${K(e)}">${G(e)}</option>`).join(``)}
                </select>
          ...
          <div class="card-body">
            ...
            <div class="form-group">
              ...
              <select id="est-discount-preset">
                ...
                ${c.map(e=>`<option value="${K(e.id)}" data-amount="${Number(e.amount)||0}" data-name="${K(e.name||`Discount`)}">${G(e.name||`Discount`)} - ${Ce(e.amount)}</option>`).join(``)}
              </select>
            </div>
            <div class="estimator-grid">
    ...
    `;let l=[],d=t=>e.querySelector(t),p=Ce,m=d(`#est-selected-services`),h=d(`#estimate-slip`),g=d(`#est-svc-main`),_=d(`#est-svc-sub`),v=d(`#est-svc-leaf`),y=d(`#est-svc-preview`),b=d(`#est-svc-add`),x=d(`#est-company`),S=d(`#est-company-custom`),C=()=>x.value===`Other`?S.value.trim():x.value.trim(),w=d(`#est-svc-search`),T=d(`#est-svc-results`);if(w){let n=(t||[]).map(e=>({id:e.id,leaf:e.sub_sub_category||e.name||``,sub:e.sub_category&&e.sub_category.trim()||``,main:e.category||``,cost:Number(e.cost)||0})).filter(e=>e.leaf),r=()=>{T.style.display=`none`,T.innerHTML=``};w.oninput=()=>{let e=w.value.trim().toLowerCase();if(e.length<1){r();return}let t=n.filter(t=>t.leaf.toLowerCase().includes(e)||t.sub.toLowerCase().includes(e)||t.main.toLowerCase().includes(e)).slice(0,10);if(!t.length){T.innerHTML=`<div style="padding:12px 16px;color:var(--text-dim);font-size:0.87rem;">No services found</div>`,T.style.display=`block`;return}T.innerHTML=t.map((e,t)=>`
          <div class="est-svc-result" data-idx="${t}" style="padding:10px 16px;cursor:pointer;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;gap:12px;transition:background .12s;">
            <div style="min-width:0;">
              <div style="font-weight:700;font-size:0.88rem;">${G(e.leaf)}</div>
              <div style="font-size:0.75rem;color:var(--text-dim);">${G([e.main,e.sub].filter(Boolean).join(` › `))}</div>
            </div>
        ...
        <div class="est-selected-row">
          <div><b>${G(e.leaf)}</b><small>${G([e.main,e.sub].filter(Boolean).join(` > `))}</small></div>
          <span>${p(e.cost)}</span>
...
`)},O=()=>{let e=E(),t=d(`#est-client-name`).value.trim()||`Client`,n=d(`#est-service-title`).value.trim()||`Service estimate`;h.innerHTML=`
        ...
        <div class="estimate-client">
          <b>${G(t)}</b>
          <small>${G(n)}${d(`#est-location`).value.trim()?` - ${G(d(`#est-location`).value.trim())}`:``}</small>
        </div>
        <div class="estimate-items">
          ${l.length?l.map(e=>`<div><span>${G(e.leaf)}</span><b>${p(e.cost)}</b></div>`).join(``):`<div><span>No service selected</span><b>Rs.0</b></div>`}
        ...
        <div class="bill-breakdown estimate-breakdown">
          ...
          ${e.discount>0?`<div class="bill-row bill-row-discount"><span>${G(e.discountLabel||`Discount`)}</span><b>-${p(e.discount)}</b></div>`:``}
    ...
    <div class="emp-job-card" data-status="${t}" data-reopened="${Number(e.reopened)===1?`1`:`0`}" data-company="${e._company||``}" style="padding:20px; border-radius:20px; background:var(--bg); box-shadow:var(--neu-sm); margin-bottom:20px; border:1px solid var(--border);${Number(e.reopened)===1?`border-left:4px solid var(--warning);`:``}">
       ...
       <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:16px;">
         <div>
           ...
           ${e.employee_update_detail?`<div style="font-size:0.82rem;color:var(--text-soft);margin-top:8px;padding:10px;border-radius:10px;background:var(--bg-soft);"><b>Employee update:</b> ${G(e.employee_update_detail)}</div>`:``}
         </div>
       ...
       <div style="margin-top:20px; display:flex; gap:10px; flex-wrap:wrap;">
         ...
         ${e.phone?`
           <a href="tel:${K(e.phone)}" class="btn btn-secondary btn-sm" style="flex:1; min-width:100px; height:40px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:8px; text-decoration:none;">
         ...
         `:``}
         <button class="btn btn-primary btn-sm" onclick="window.open('${K(te(e))}')" style="flex:1; min-width:120px; height:40px; font-weight:700; display:flex; align-items:center; justify-content:center; gap:8px;">
  ...
  `},w=e=>{let t=e.inquiries?.[0],n=P(e.status);return`
      <div class="emp-task-card" data-status="${n}" data-reopened="${Number(t?.reopened)===1?`1`:`0`}" style="padding:20px; border-radius:20px; background:var(--bg); box-shadow:var(--neu-sm); margin-bottom:20px; border:1px solid var(--border);${Number(t?.reopened)===1?`border-left:4px solid var(--warning);`:``}">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
              ...
              ${t.employee_update_detail?`
                <div style="grid-column: span 2">
                  ...
                  <div style="font-size:0.88rem; font-weight:600; white-space:pre-wrap;">${G(t.employee_update_detail)}</div>
                </div>
              `:``}
              <div style="grid-column: span 2">
        ...
        <div style="margin-top:16px; display:flex; gap:10px; flex-wrap:wrap;">
          ...
          ${t&&t.phone?`
            <a href="tel:${K(t.phone)}" class="btn btn-secondary btn-sm" style="flex:1; min-width:100px; height:42px; display:flex; align-items:center; justify-content:center; gap:6px; font-weight:700; text-decoration:none;">
              <span style="width:16px;height:16px;display:flex;">${u.phone}</span> Call
            </a>
          ...
          `:``}
          ${t?`
            <button class="btn btn-primary btn-sm" onclick="window.open('${K(te(t))}')" style="flex:1; min-width:120px; height:42px; display:flex; align-items:center; justify-content:center; gap:6px; font-weight:700;">
              <span style="width:16px;height:16px;display:flex;">${u.pin}</span> Open Maps
            </button>
          `:``}
        </div>
      </div>
    `};[...g,...p].sort(W),[..._,...m].sort(W),[...v,...h].sort(W),e.innerHTML=`
    <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;">
      <div>
            ...
            <div style="background:linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(16, 185, 129, 0.05) 100%); border:2px solid var(--primary); border-radius:16px; padding:20px; box-shadow:0 4px 12px rgba(16, 185, 129, 0.1);">
              <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:14px;">
                <div>
                  ...
                  <div style="font-size:1.1rem; font-weight:800; color:var(--text);">${G(e.full_name)}</div>
                </div>
                <span style="background:var(--primary); color:white; padding:6px 12px; border-radius:20px; font-size:0.75rem; font-weight:700;">${G(e.service_item)}</span>
              </div>
              ...
              <div style="background:rgba(16, 185, 129, 0.08); border-left:3px solid var(--primary); padding:12px; border-radius:8px; margin-bottom:14px;">
                ...
                <div style="font-size:0.95rem; font-weight:700; color:var(--primary);">${G(e.ticket_no||`NE-`+Math.random().toString(36).substring(2,10).toUpperCase())}</div>
              </div>
              ...
              <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:14px;">
                ...
                <div>
                  ...
                  <div style="font-size:0.85rem; font-weight:600; color:var(--primary);">${G(e.preferred_time||`Flexible`)}</div>
                </div>
              </div>
              ...
              <div style="margin-bottom:14px;">
                ...
                <div style="font-size:0.85rem; font-weight:600; display:flex; align-items:flex-start; gap:6px;">
                  ...
                  <span>${G(e.location||`Not specified`)}</span>
                </div>
              </div>
  ...
  `,j(e,t.id,{deferShow:!0});let T=null;e._tasksChannel=f.channel(`employee-tasks-${t.id}`).on(`postgres_changes`,{event:`*`,schema:`public`,table:`inquiries`,filter:`assigned_employee_id=eq.${t.id}`},t=>{let n=t.new;(t.eventType===`INSERT`||t.eventType===`UPDATE`&&n?.assignment_status===`pending`)&&r({title:`🔔 New Job Assigned`,body:`${n?.full_name||`A client`} - ${n?.service_item||`new service`}`,type:`alert`,tag:`assign-${n?.id||``}`}),clearTimeout(T),T=setTimeout(()=>{document.body.contains(e)&&Pe(e)},400)}).subscribe();let E=setInterval(()=>{if(!document.body.contains(e)){try{f.removeChannel(e._tasksChannel)}catch{}e._tasksChannel=null,clearInterval(E)}},5e3),ee={active:`In Progress Services`,in_progress:`In Progress Services`,reopened:`🔁 Reopened Tickets (free rework)`,resolved:`Resolved Services`,issue_not_resolved:`Issue Not Resolved Services`,case_closed:`Case Closed Services`},D=`in_progress`,O=``,k=()=>{let t=e.querySelector(`#services-card`),n=e.querySelector(`#devices-in-service-card`),r=O.toLowerCase();if(D===`device_followup`){if(t&&(t.style.display=`none`),n&&(n.style.display=`block`,n.dataset.hasDevices!==`1`)){let e=n.querySelector(`#devices-in-service-list`);e&&(e.innerHTML=`<div style="text-align:center;padding:28px;color:var(--text-dim)">No devices in the service center right now.</div>`)}return}n&&(n.style.display=`none`),t&&(t.style.display=``);let i=e.querySelector(`#services-card-title`);i&&(i.textContent=ee[D]||`Services`);let a=0;e.querySelectorAll(`#services-list .emp-task-card, #services-list .emp-job-card`).forEach(e=>{let t=y(e.dataset.status),n=(D===`reopened`?e.dataset.reopened===`1`:D===`in_progress`?t===`in_progress`||t===`active`:t===D)&&(!r||e.textContent.toLowerCase().includes(r));e.style.display=n?``:`none`,n&&a++});let o=e.querySelector(`#services-empty`);o&&(o.style.display=a?`none`:``)},A=e.querySelector(`#task-panel`),M=e.querySelector(`#task-filter-btn`),N=e.querySelector(`#task-badge`),F=()=>{let e=(D===`in_progress`?0:1)+ +!!O;N.textContent=e,N.style.display=e?``:`none`},I=e=>{M.closest(`.df-wrap`).contains(e.target)||(A.style.display=`none`,M.classList.remove(`df-open`),document.removeEventListener(`click`,I))};M.onclick=e=>{e.stopPropagation(),A.style.display===`none`?(A.style.display=``,M.classList.add(`df-open`),setTimeout(()=>document.addEventListener(`click`,I),0)):(A.style.display=`none`,M.classList.remove(`df-open`),document.removeEventListener(`click`,I))};let L=e.querySelector(`#task-search`),R=()=>{A.style.display=`none`,M.classList.remove(`df-open`),document.removeEventListener(`click`,I)};e.querySelector(`#task-ok`).onclick=()=>{D=e.querySelector(`#task-status-sel`).value,O=L?L.value:``,k(),F(),R()},e.querySelector(`#task-filter-clear`).onclick=()=>{D=`in_progress`,O=``,e.querySelector(`#task-status-sel`).value=`in_progress`,L&&(L.value=``),k(),F(),R()},k(),e.querySelectorAll(`.task-btn`).forEach(t=>{t.onclick=()=>ne(t,t.dataset.id,t.dataset.inqId,t.dataset.status,()=>Pe(e))}),e.querySelectorAll(`.accept-btn`).forEach(t=>{t.onclick=async()=>{let r=[f.from(`inquiries`).update({assignment_status:`accepted`,status:`in_progress`}).eq(`id`,t.dataset.id)];t.dataset.ticketId&&r.push(f.from(`tickets`).update({status:`in_progress`}).eq(`id`,t.dataset.ticketId));let i=(await Promise.all(r)).find(e=>e.error)?.error;i?n(i.message,`error`):(n(`Task accepted!`,`success`),Pe(e))}}),e.querySelectorAll(`.decline-btn`).forEach(t=>{t.onclick=()=>{let r=prompt(`Please provide a reason for declining:`);if(r!==null){if(!r.trim()){n(`Reason is required to decline`,`warning`);return}(async()=>{let{error:i}=await f.from(`inquiries`).update({assignment_status:`declined`,decline_reason:r.trim(),status:`open`}).eq(`id`,t.dataset.id);i?n(i.message,`error`):(n(`Task declined`,`info`),Pe(e))})()}}})}function Fe(e,a,o,d){return(async()=>{let{data:p}=await f.from(`service_pricing`).select(`*`).order(`category`),{data:_}=await f.from(`discount_presets`).select(`*`).eq(`active`,1).order(`created_at`,{ascending:!1}),{data:v}=await f.from(`device_types`).select(`name`).order(`name`),y=Array.isArray(v)?v:[],{data:D}=await f.from(`companies`).select(`*`).order(`name`),k=Array.isArray(D)?D:[],ne=Array.isArray(_)?_:[],A={status:`unpaid`,received_at:null},j=null;if(a){let{data:e}=await f.from(`inquiries`).select(`*`).eq(`id`,a).single();e&&(j=e,A={status:e.payment_status||`unpaid`,received_at:e.payment_received_at||null})}if(!j&&e){let{data:t}=await f.from(`inquiries`).select(`*`).eq(`ticket_id`,e).maybeSingle();t&&(j=t,a=t.id,A={status:t.payment_status||`unpaid`,received_at:t.payment_received_at||null})}let{data:{user:M}}=await f.auth.getUser(),N=M?(await f.from(`profiles`).select(`*`).eq(`id`,M.id).single()).data:null,F=new Date().toLocaleDateString(`en-CA`),I={lat:null,lng:null,location:null};if(M){let{data:e}=await f.from(`attendance`).select(`latitude,longitude,location,clock_in`).eq(`user_id`,M.id).eq(`date`,F).order(`clock_in`,{ascending:!1});Array.isArray(e)&&e[0]&&(I={lat:e[0].latitude,lng:e[0].longitude,location:e[0].location})}let L={};(p||[]).forEach(e=>{let t=e.category||`Uncategorized`,n=e.sub_category&&e.sub_category.trim()||``,r=e.sub_sub_category||e.name||``;r&&(L[t]??={},L[t][n]??=[],L[t][n].push({id:e.id,leaf:r,cost:Number(e.cost)||0}))}),Object.keys(L).sort();let R=P(o),z=T(R),ie=R===`resolved`;await w();let ae=C,B=Number(j?.device_service_enabled)===1,oe=Number(j?.device_service_enabled)!==1||j?.follow_up_status===`returned`||j?.device_status===`returned`,se=j?c(j):null,de=s(j),fe=(()=>{if(!j?.scheduled_at)return``;let e=new Date(j.scheduled_at);return Number.isNaN(e.getTime())?``:new Date(e.getTime()-e.getTimezoneOffset()*6e4).toISOString().slice(0,16)})();we(j?.created_at,j?.updated_at||new Date),[`resolved`,`closed`,`issue_not_resolved`].includes(P(j?.status))&&we(j?.created_at,j?.updated_at||j?.bill_generated_at||new Date);let V=[];if(a){let{data:e}=await f.from(`inquiry_services`).select(`service_id, service_pricing(name, category, sub_category, sub_sub_category, cost)`).eq(`inquiry_id`,a);if(Array.isArray(e)){let t=new Set;e.forEach(e=>{if(!e.service_id||t.has(e.service_id))return;t.add(e.service_id);let n=e.service_pricing||{};V.push({id:e.service_id,main:n.category||`Service`,sub:n.sub_category||``,leaf:n.sub_sub_category||n.name||``,cost:Number(n.cost)||0})})}}let pe=()=>V.reduce((e,t)=>e+(Number(t.cost)||0),0),me=()=>Math.max(0,Number(j?.bill_amount)||0),he=()=>{let e=pe();return e>0?e:me()},ge=0;if(j){let e=he(),t=Number(j.extra_cost)||0,n=Number(j.transport_km)||0,r=Math.round(n*5),i=(j.company_name||`networking experts`).toLowerCase().replace(/\s+/g,` `)===`networking experts`?50:100,a=e+t+i+r>250?30:0,o=j.discount_preset_id,s=ne.find(e=>e.id===o),c=s&&Number(s.amount)||0,l=Number(j.discount_amount)||0;ge=Math.max(0,l-a-c)}let H=document.createElement(`div`);H.className=`modal-overlay`,H.innerHTML=`
        ...
        <div class="modal-body" style="padding-top:14px;">
            ...
            <div style="margin:0 0 16px;padding:16px;border-radius:14px;background:var(--bg-soft);border:1px solid var(--border);">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:14px;">
                <div>
                  ...
                  <div style="font-size:0.95rem;font-weight:700;color:var(--primary);">${G(j.ticket_no||`No ticket`)}</div>
                </div>
              ...
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
                <div>
                  ...
                  <div style="font-size:0.88rem;font-weight:600;">${G(j.full_name||`Client`)}</div>
                </div>
                <div>
                  ...
                  <div style="font-size:0.88rem;font-weight:600;">${G(j.phone||`-`)}</div>
                </div>
              </div>
              <div style="margin-bottom:14px;">
                ...
                <div style="font-size:0.88rem;font-weight:600;">${G(j.service_item||`Service request`)}</div>
              </div>
              ${j.description?`
                <div style="margin-bottom:14px;padding:10px;background:rgba(16,185,129,0.05);border-radius:10px;border-left:3px solid var(--primary);">
                  ...
                  <div style="font-size:0.82rem;line-height:1.4;color:var(--text);">${G(j.description)}</div>
                </div>
              `:``}
              <div style="margin-bottom:14px;">
                ...
                <div style="font-size:0.88rem;font-weight:600;display:flex;align-items:flex-start;gap:6px;">
                  ...
                  <span style="line-height:1.4;">${G(j.location||`-`)}</span>
                </div>
              </div>
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                <div>
                  ...
                  <div style="font-size:0.88rem;font-weight:600;color:var(--primary);">${G(j.preferred_time||`Flexible`)}</div>
                </div>
                <div>
                  ...
                  <div style="font-size:0.88rem;font-weight:600;">${[`resolved`,`closed`,`issue_not_resolved`,`foc`].includes(j?.status)?`Service Completed`:de?`<span style="color:var(--warning)">⏸ Paused — device in service</span>`:se?t(se):`-`}</div>
                </div>
              </div>
            </div>
          `:``}
          ...
          ${j?`
            <div style="background:var(--bg-soft);border:1px solid var(--border);border-radius:14px;padding:14px;margin:0 0 12px;">
              ...
              <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px 16px;font-size:0.85rem;margin-bottom:12px;">
                ${[[`Name`,j.full_name],[`Phone`,j.phone],[`Ticket`,j.ticket_no],[`Service`,j.service_item],[`Company`,j.company_name],[`Address`,j.address||j.location]].filter(([,e])=>e).map(([e,t])=>`<div><div style="color:var(--text-dim);font-size:0.7rem;font-weight:700;text-transform:uppercase;">${e}</div><div style="font-weight:600;">${G(t)}</div></div>`).join(``)}
              </div>
              <div style="display:flex;gap:8px;flex-wrap:wrap;">
                ${j.phone?`
                  <a href="tel:${K(j.phone)}" class="btn btn-secondary btn-sm" style="flex:1;min-width:90px;height:38px;display:flex;align-items:center;justify-content:center;gap:6px;text-decoration:none;font-weight:700;"><span style="width:15px;height:15px;display:flex;">${u.phone}</span>Call</a>
                ...
                `:``}
                <a href="${K(te(j))}" target="_blank" rel="noopener" class="btn btn-primary btn-sm" style="flex:1;min-width:90px;height:38px;display:flex;align-items:center;justify-content:center;gap:6px;text-decoration:none;font-weight:700;"><span style="width:15px;height:15px;display:flex;">${u.pin}</span>Open Map</a>
              </div>
            </div>
          `:``}

          ${I.lat!=null||j?.customer_lat!=null?`
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:0 0 12px;">
              <a href="${I.lat==null?`#`:K(O(I.lat,I.lng))}" target="_blank" rel="noopener"
                 style="padding:10px;border-radius:10px;background:var(--bg-soft);border:1px solid var(--border);text-decoration:none;color:var(--text);font-size:0.78rem;${I.lat==null?`pointer-events:none;opacity:.55;`:``}">
                <b style="display:block;color:var(--primary);margin-bottom:3px;">Employee pin</b>
                <span>${G(I.location||`Clock-in GPS`)}</span>
              </a>
              <a href="${j?.customer_lat==null?`#`:K(O(j.customer_lat,j.customer_lng))}" target="_blank" rel="noopener"
                ...
                <span>${G(j?.location||`Customer GPS`)}</span>
              </a>
            </div>
          `:``}
          ...
          ${j?.description?`
            <div style="margin:0 0 14px;padding:12px;border-radius:10px;background:var(--bg-soft);border:1px solid var(--border);">
              ...
              <div style="white-space:pre-wrap;line-height:1.45;font-size:0.86rem;">${G(j.description)}</div>
            </div>
          `:``}
          ...
          <div class="mst-pane active" data-pane="status">
            ...
            <div class="form-group">
              ...
              <select id="new-status" ${z?`disabled`:``}>
                ${Number(j?.reopened)===1?`
                  <option value="resolved" ${R===`resolved`?`selected`:``}>Resolved</option>
                  <option value="foc" ${R===`foc`?`selected`:``}>FOC — Free of Cost (no bill generated)</option>
                `:`
                  <option value="in_progress" ${R===`in_progress`?`selected`:``}>In Progress</option>
                  <option value="resolved" ${R===`resolved`?`selected`:``}>Resolved</option>
                  <option value="reschedule">📅 Reschedule (set new visit time)</option>
                  <option value="issue_not_resolved" ${R===`issue_not_resolved`?`selected`:``}>Issue Not Resolved</option>
                  <option value="case_closed" ${R===`case_closed`?`selected`:``}>Case Closed — customer didn't cooperate / no fee</option>
                `}
              </select>
              <small id="case-closed-hint" style="display:${R===`case_closed`?`block`:`none`}; margin-top:6px; color:var(--danger); font-size:0.75rem;">⚠️ Case Closed is final — the ticket will be locked and cannot be reopened.</small>
            </div>

            <div class="form-group" id="foc-billno-group" style="display:${R===`foc`?`block`:`none`};">
              <label>Client Bill Number <span style="color:var(--danger)">*</span></label>
              <input type="text" id="foc-bill-no" value="${G(j?.bill_no||``)}" placeholder="Enter the customer's existing bill number" />
          ...
          <div class="mst-pane" data-pane="device">
            <div class="form-group">
              ...
              <select id="resolve-company" style="margin-bottom:8px;" ${z?`disabled`:``}>
              ...
              <input type="text" id="resolve-company-custom" placeholder="Type custom company name (Mandatory)" style="display:none;" ${z?`disabled`:``}/>
            </div>
            <div class="form-group">
              <label>Device Type</label>
              <input type="text" id="device-type" list="emp-device-types" placeholder="${y.length?`Start typing or pick...`:`e.g. Video Door Phone`}" value="${(j?.device_type||``).replace(/"/g,`&quot;`)}" ${z?`disabled`:``}/>
            ...
            <div class="form-group">
              ...
              <input type="text" id="device-serial" placeholder="e.g. SN-12345" value="${(j?.device_serial_no||``).replace(/"/g,`&quot;`)}" ${z?`disabled`:``}/>
            </div>
          ...
          <div class="mst-pane" data-pane="service">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;border-radius:12px;background:var(--bg-soft);border:1px solid var(--border);margin-bottom:14px;">
              ...
              <label class="switch">
                <input type="checkbox" id="device-service-toggle" ${B?`checked`:``}>
                <span class="switch-slider"></span>
              </label>
            </div>

            <div id="device-service-body" style="display:${B?`block`:`none`};">
              <div class="form-group">
          ...
          <div class="mst-pane" data-pane="bill">
            ...
            <div id="pricing-section" style="display:${ie?`block`:`none`};">
              ...
              <div class="form-group">
                ...
                <input type="text" id="extra-reason" placeholder="Reason for extra charge..." value="${G(j?.extra_cost_reason||``)}"/>
              </div>
              ...
              <div class="form-group">
                ...
                <select id="admin-discount-preset" class="svc-picker" style="margin-bottom:8px;">
                  <option value="">No admin discount</option>
                  ${ne.map(e=>{let t=j?.discount_preset_id===e.id;return`<option value="${K(e.id)}" data-amount="${Number(e.amount)||0}" data-name="${K(e.name||`Discount`)}" ${t?`selected`:``}>${G(e.name||`Discount`)} - Rs.${Math.round(Number(e.amount)||0).toLocaleString(`en-IN`)}</option>`}).join(``)}
                </select>
                <div style="display:grid;grid-template-columns:1fr 1.4fr;gap:8px;">
                  <input type="number" id="manual-discount" min="0" step="1" placeholder="Employee discount Rs.0" value="${ge||``}"/>
                  <input type="text" id="discount-reason" placeholder="Reason required for employee discount" value="${G(j?.discount_reason||``)}"/>
                </div>
              ...
              <div id="bill-pdf-actions" style="display:${j?.bill_pdf_url?`block`:`none`}; margin-bottom:14px; padding:14px; border-radius:14px; background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.35);">
                ...
                <div style="display:flex; gap:8px; flex-wrap:wrap;">
                  <a class="btn btn-secondary btn-sm" id="bill-pdf-view" href="${K(j?.bill_pdf_url||`#`)}" target="_blank" rel="noopener" style="text-decoration:none;">${u.receipt}<span>View PDF</span></a>
              ...
              <div style="padding:14px; background:var(--bg-soft); border-radius:14px; border:1px solid var(--border);">
                ...
                <div style="display:flex; gap:8px; margin-bottom:10px;">
                  ...
                  <button class="btn btn-secondary btn-sm" id="emp-gen-link" style="white-space:nowrap">${u.link}<span>Generate</span></button>
                </div>
      ...
      </div>`,document.body.appendChild(H);let _e=()=>H.remove();if(H.querySelector(`#cm`)?.addEventListener(`click`,_e),H.querySelector(`#cm2`)?.addEventListener(`click`,_e),H.addEventListener(`click`,e=>{e.target===H&&_e()}),j?.employee_update_detail){let e=document.createElement(`div`);e.style.cssText=`padding:12px;border-radius:12px;background:var(--bg-soft);border:1px solid var(--border);margin:12px 0;`,e.innerHTML=`<div class="sr-meta-label">Last submitted detail</div><div class="sr-meta-value" style="white-space:pre-wrap;line-height:1.45;">${G(j.employee_update_detail)}</div><small style="color:var(--text-dim)">Status: ${re(j.employee_update_status||j.status)}${j.employee_update_at?` - ${l(j.employee_update_at)}`:``}</small>`,H.querySelector(`[data-pane="status"]`)?.appendChild(e)}let ve=[`status`,`device`,`bill`],ye=()=>H.querySelector(`.mst-tab.active`)?.dataset.tab||ve[0],be=()=>ye()===ve[ve.length-1],xe=e=>{H.querySelectorAll(`.mst-tab`).forEach(t=>t.classList.toggle(`active`,t.dataset.tab===e)),H.querySelectorAll(`.mst-pane`).forEach(t=>t.classList.toggle(`active`,t.dataset.pane===e)),Q()};H.querySelectorAll(`.mst-tab`).forEach(e=>{e.onclick=()=>xe(e.dataset.tab)}),(()=>{let e=H.querySelector(`[data-pane="service"]`),t=H.querySelector(`[data-pane="status"]`);if(!e||!t)return;e.classList.remove(`mst-pane`),e.removeAttribute(`data-pane`),e.style.display=`block`;let n=H.querySelector(`#device-serial`)?.closest(`.form-group`),r=e.querySelector(`#device-service-body`);n&&r&&r.insertBefore(n,r.firstChild);let i=document.createElement(`hr`);i.style.cssText=`border:none;border-top:1px solid var(--border);margin:18px 0 14px;`,t.appendChild(i),t.appendChild(e)})();let U=H.querySelector(`#new-status`),Se=H.querySelector(`#pricing-section`),Ce=H.querySelector(`#total-bill-display`),W=H.querySelector(`#extra-cost`),Te=H.querySelector(`#extra-reason`),Ee=H.querySelector(`#admin-discount-preset`),De=H.querySelector(`#manual-discount`),Oe=H.querySelector(`#discount-reason`),ke=H.querySelector(`#transport-km`),q=H.querySelector(`#save-update`),J=e=>`₹${Math.round(Number(e)||0).toLocaleString(`en-IN`)}`,Ae=()=>{let e=H.querySelector(`#resolve-company`),t=H.querySelector(`#resolve-company-custom`);return e?e.value===`Other`?t?.value.trim()||``:e.value:``},je=()=>Ae().toLowerCase().replace(/\s+/g,` `)===`networking experts`?50:100,Me=async e=>{if(!e)return;let t=e.trim();if(t.toLowerCase()===`networking experts`||k.some(e=>e.name.toLowerCase()===t.toLowerCase()))return;let{data:n}=await f.from(`companies`).select(`name`).eq(`name`,t).maybeSingle();if(n)return;let{error:r}=await f.from(`companies`).insert({id:crypto.randomUUID?crypto.randomUUID():String(Math.random()),name:t});r||k.push({name:t})},Y={servicesSubtotal:0,extra:0,platform:je(),km:0,transport:0,autoDiscount:0,presetDiscount:0,manualDiscount:0,discount:0,discountLabel:``,discountReason:``,discountPresetId:null,taxable:0,gst:0,total:0},Ne=j?.bill_pdf_url||``,Pe=j?.payment_link||``,Fe=()=>{let e=H.querySelector(`#bill-pdf-actions`),t=H.querySelector(`#bill-pdf-view`);e&&(e.style.display=Ne?`block`:`none`,Ne&&t&&(t.href=Ne))},Ie=(e=j||{})=>{let t=V.length?V.map(e=>({name:`${e.main}${e.sub?` > `+e.sub:``} > ${e.leaf}`,cost:e.cost})):Y.servicesSubtotal>0?[{name:e.service_item||`Service`,cost:Y.servicesSubtotal}]:[];return{...Y,customer:{name:e.full_name||``,phone:e.phone||``,location:e.location||``,company:Ae()||e.company_name||``,device_type:H.querySelector(`#device-type`)?.value.trim()||e.device_type||``,device_serial:H.querySelector(`#device-serial`)?.value.trim()||e.device_serial_no||``,service_item:e.service_item||``,ticket_no:e.ticket_no||``},technician:N?.full_name||`Technician`,services:t,extraReason:H.querySelector(`#extra-reason`)?.value.trim()||``,discountLabel:Y.discountLabel,discountReason:Y.manualDiscount>0?Y.discountReason:``,paymentLink:Pe||e.payment_link||``,paymentStatus:e.payment_status||`unpaid`}},X=()=>{Y.servicesSubtotal=he(),Y.extra=Number(W.value)||0,Y.km=Math.max(0,Number(ke.value)||0),Y.transport=Math.round(Y.km*5),Y.platform=je();let e=Y.servicesSubtotal+Y.extra+Y.platform+Y.transport;Y.autoDiscount=e>250?30:0;let t=Ee?.selectedOptions?.[0];Y.discountPresetId=Ee?.value||null,Y.presetDiscount=Y.discountPresetId&&Number(t?.dataset.amount)||0,Y.manualDiscount=Math.max(0,Number(De?.value)||0),Y.discount=Math.min(e,Y.autoDiscount+Y.presetDiscount+Y.manualDiscount);let n=[];Y.autoDiscount&&n.push(`Loyalty discount`),Y.presetDiscount&&n.push(t?.dataset.name||`Admin discount`),Y.manualDiscount&&n.push(`Employee discount`),Y.discountLabel=n.join(` + `)||``,Y.discountReason=Oe?.value.trim()||``,Y.taxable=e-Y.discount,Y.gst=Math.round(Y.taxable*.18),Y.total=Y.taxable+Y.gst;let r=H.querySelector(`#br-service-lines`);if(r){let e=V.length?V.map(e=>({label:`${e.main}${e.sub?` > `+e.sub:``} > ${e.leaf}`,cost:Number(e.cost)||0})):Y.servicesSubtotal>0?[{label:j?.service_item||`Service`,cost:Y.servicesSubtotal}]:[];r.innerHTML=e.length?e.map(e=>`
              <div class="bill-row">
                <span>${G(e.label)}</span>
                <b>${J(e.cost)}</b>
              </div>
            `).join(``):`<div class="bill-row"><span>No service added</span><b>${J(0)}</b></div>`}let i=H.querySelector(`#br-extra-row`),a=H.querySelector(`#br-extra-label`);if(i&&(i.style.display=Y.extra>0?`flex`:`none`),a){let e=Te?.value.trim();a.textContent=e?`Additional charges (${e})`:`Additional charges`}H.querySelector(`#br-extra`).textContent=J(Y.extra),H.querySelector(`#br-platform`).textContent=J(Y.platform),H.querySelector(`#br-km`).textContent=Y.km.toString(),H.querySelector(`#br-transport`).textContent=J(Y.transport);let o=H.querySelector(`#br-discount-row`);o.style.display=Y.discount>0?`flex`:`none`,o.querySelector(`span`).textContent=Y.discountLabel||`Discount`,H.querySelector(`#br-discount`).textContent=`-${J(Y.discount)}`,H.querySelector(`#br-gst`).textContent=J(Y.gst),H.querySelector(`#br-total`).textContent=J(Y.total),Ce.value=String(Y.total);let s=H.querySelector(`#cash-amount-display`);s&&(s.textContent=J(Y.total))},Le=()=>(X(),Y.manualDiscount>0&&!Y.discountReason?(n(`Please enter reason for employee discount`,`warning`),!1):!0),Re=(e,t,n,r)=>{let i=e=>e*Math.PI/180,a=i(n-e),o=i(r-t),s=Math.sin(a/2)**2+Math.cos(i(e))*Math.cos(i(n))*Math.sin(o/2)**2;return 2*6371*Math.asin(Math.sqrt(s))},Z={lat:null,lng:null,accuracy:null},ze=H.querySelector(`#auto-km`),Be=H.querySelector(`#capture-loc`),Ve=H.querySelector(`#transport-km-hint`),He=H.querySelector(`#bill-loc-status`),Ue=()=>{let e=I.lat,t=I.lng,n=Z.lat!=null,r=j?.customer_lat!=null&&j?.customer_lng!=null;if(e==null||t==null){Ve.textContent=`₹5 per km  ?  enter manually (no clock-in GPS on record for today).`,ze.disabled=!0,ze.style.opacity=`0.5`;return}if(!n&&!r){Ve.textContent=`₹5 per km  ?  capture your precise location at the customer site to enable Auto km.`,ze.disabled=!0,ze.style.opacity=`0.5`;return}ze.disabled=!1,ze.style.opacity=`1`,Ve.textContent=n?`Start point: your clock-in location (fixed for today) → on-site capture. ₹5/km, auto-filled.`:`Start point: your clock-in location (fixed for today) → customer GPS. ₹5/km, auto-filled. Capture on-site for best accuracy.`};Ue(),Be.onclick=async()=>{Be.disabled=!0;let e=Be.textContent;Be.innerHTML=`<span class="srf-spin"></span> Locating...`;try{let{latitude:e,longitude:t,accuracy:r}=(await ee()).coords;Z.lat=e,Z.lng=t,Z.accuracy=r,He.style.display=`block`,He.textContent=`📍 Captured: ${e.toFixed(6)}, ${t.toFixed(6)} (±${Math.round(r)}m)`,n(`Precise location captured (±${Math.round(r)}m)`,`success`),Ue()}catch(e){n(`Could not capture location - check GPS permission`,`error`),console.error(`Capture location failed:`,e)}finally{Be.disabled=!1,Be.textContent=e}};let We=({silent:e}={})=>{let t=I.lat,r=I.lng;if(t==null||r==null)return!1;let i,a,o;if(Z.lat!=null&&Z.lng!=null)i=Z.lat,a=Z.lng,o=`on-site capture`;else if(j?.customer_lat!=null&&j?.customer_lng!=null)i=j.customer_lat,a=j.customer_lng,o=`customer GPS`;else return!1;let s=Re(Number(t),Number(r),Number(i),Number(a))*1.3;return ke.value=s.toFixed(1),X(),e||Q(),e||n(`Distance: ${s.toFixed(1)} km (clock-in location → ${o})`,`success`),!0};ze.onclick=()=>We({silent:!1}),(!ke.value||Number(ke.value)===0)&&We({silent:!0}),ke.oninput=()=>{X(),Q()};let Ge=H.querySelector(`#progress-detail`);Ge&&(Ge.oninput=()=>{Q()});let Ke=H.querySelector(`#resolve-company`),qe=H.querySelector(`#resolve-company-custom`),Je=()=>{Ke.value===`Other`?qe.style.display=`block`:(qe.style.display=`none`,qe.value=``),X(),Q()};if(Ke){Ke.onchange=Je;let e=j?.company_name||`networking experts`;k.some(t=>t.name.toLowerCase()===e.toLowerCase())?(Ke.value=k.find(t=>t.name.toLowerCase()===e.toLowerCase())?.name||e,qe.style.display=`none`,qe.value=``):(Ke.value=`Other`,qe.style.display=`block`,qe.value=e)}qe&&(qe.oninput=()=>{X(),Q()});let Ye=H.querySelector(`#svc-selected`),Xe=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]),Ze=()=>{if(Ye){if(V.length===0){Ye.style.display=`none`,Ye.innerHTML=``;return}Ye.style.display=`block`,Ye.innerHTML=`
        <div style="font-size:0.78rem; font-weight:700; color:var(--text-dim); margin-bottom:6px;">Services performed (${V.length})</div>
        ${V.map((e,t)=>`
          <div style="display:flex; align-items:center; gap:8px; padding:8px 10px; background:var(--bg); border:1px solid var(--border); border-radius:10px; margin-bottom:6px;">
            <div style="flex:1; font-size:0.85rem;">
              <b>${Xe(e.main)}</b>${e.sub?` › ${Xe(e.sub)}`:``} › ${Xe(e.leaf)}
            </div>
            <span style="font-weight:700; color:var(--primary);">₹${Number(e.cost).toLocaleString(`en-IN`)}</span>
            <button type="button" class="btn btn-danger btn-sm svc-remove" data-idx="${t}" title="Remove" ${z?`style="display:none;" disabled`:``}>✕</button>
          </div>
        `).join(``)}
      `,Ye.querySelectorAll(`.svc-remove`).forEach(e=>{e.onclick=()=>{V.splice(Number(e.dataset.idx),1),Ze(),X(),Q()}})}},Qe=(p||[]).map(e=>({id:e.id,main:e.category||`Uncategorized`,sub:(e.sub_category||``).trim(),leaf:e.sub_sub_category||e.name||``,cost:Number(e.cost)||0,_s:[e.category,e.sub_category,e.sub_sub_category,e.name].filter(Boolean).join(` `).toLowerCase()})).filter(e=>e.leaf),$e=H.querySelector(`#svc-search`),et=H.querySelector(`#svc-results`),tt=()=>{et&&(et.innerHTML=``,et.style.display=`none`)},nt=e=>{if(V.some(t=>t.id===e.id)){n(`Already added`,`warning`);return}V.push({id:e.id,main:e.main,sub:e.sub,leaf:e.leaf,cost:e.cost}),Ze(),X(),Q(),$e&&($e.value=``),tt()};$e&&($e.addEventListener(`input`,()=>{let e=$e.value.trim().toLowerCase();if(!e){tt();return}let t=e.split(/\s+/),n=Qe.filter(e=>t.every(t=>e._s.includes(t))).slice(0,10);if(n.length===0){et.innerHTML=`<div class="svc-result-empty">No services match "<b>${Xe(e)}</b>"</div>`,et.style.display=`block`;return}et.innerHTML=n.map((e,t)=>`
          <div class="svc-result-item" data-idx="${t}">
            <div class="svc-result-name">${Xe(e.main)}${e.sub?` <span class="svc-result-sep">›</span> ${Xe(e.sub)}`:``} <span class="svc-result-sep">›</span> <b>${Xe(e.leaf)}</b></div>
            <span class="svc-result-price">₹${e.cost.toLocaleString(`en-IN`)}</span>
          </div>`).join(``),et.style.display=`block`,et.querySelectorAll(`.svc-result-item`).forEach((e,t)=>{e.addEventListener(`mousedown`,e=>{e.preventDefault(),nt(n[t])})})}),$e.addEventListener(`blur`,()=>setTimeout(tt,180)),$e.addEventListener(`focus`,()=>{$e.value.trim()&&$e.dispatchEvent(new Event(`input`))}));let rt=H.querySelector(`#emp-pay-status`),it=H.querySelector(`#emp-pay-status-icon`),at=H.querySelector(`#emp-pay-status-title`),ot=H.querySelector(`#emp-pay-status-sub`),st=!1,ct=!1,Q=()=>{let e=U.value===`resolved`,t=Number(Ce.value)||0,n=e&&t>0,r=A.status===`paid`;r?(rt.style.borderColor=`var(--success)`,rt.style.background=`rgba(16,185,129,0.08)`,it.innerHTML=u.check,it.style.color=`var(--success)`,at.textContent=`✓ Payment Received`,at.style.color=`var(--success)`,ot.textContent=A.received_at?`Received at ${i(A.received_at)} - you can now submit.`:`You can now submit the resolution.`):st?(rt.style.borderColor=`var(--warning)`,rt.style.background=`rgba(245,158,11,0.06)`,it.innerHTML=u.clock,it.style.color=`var(--warning)`,at.textContent=`⏳ Waiting for Payment`,at.style.color=`var(--warning)`,ot.textContent=`Auto-checking every 3s - Save unlocks the moment Razorpay confirms.`):(rt.style.borderColor=`var(--border)`,rt.style.background=`var(--bg)`,it.innerHTML=u.clock,it.style.color=`var(--text-dim)`,at.textContent=`Payment: ${A.status===`paid`?`Paid`:`Unpaid`}`,at.style.color=`var(--text)`,ot.textContent=n?`Generate a link, then wait for the client to pay before submitting.`:`Generate a link, then wait for the client to pay.`);let a=H.querySelector(`#progress-detail`)?.value.trim()||``,o=Ae(),s=a.length>0,c=o.length>0;if(z)q.disabled=!0,q.textContent=be()?`Resolved`:`Next →`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=`Resolved tasks are read-only.`;else if(!e){let e=U.value===`foc`,t=U.value===`reschedule`,n=H.querySelector(`#foc-bill-no`)?.value.trim()||``,r=H.querySelector(`#reschedule-at`)?.value||``;t?r?(q.disabled=!1,q.textContent=`📅 Save Schedule`,q.style.opacity=`1`,q.style.cursor=`pointer`,q.title=``):(q.disabled=!0,q.textContent=`Pick a date & time`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=`Choose the new visit date & time first.`):s?e&&!n?(q.disabled=!0,q.textContent=`Enter client bill number`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=`FOC requires the client's bill number before you can complete the service.`):e&&B&&!oe?(q.disabled=!0,q.textContent=`Return device first`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=`This ticket has a device in service. Mark the device as returned before closing the service.`):(q.disabled=!1,q.textContent=e?`Complete (FOC)`:`Save Changes`,q.style.opacity=`1`,q.style.cursor=`pointer`,q.title=``):(q.disabled=!0,q.textContent=`Save Changes`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=`Please fill out the Work Details / Progress Update first.`)}else if(be())!s||!c?(q.disabled=!0,q.textContent=`Save Changes`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=`Please fill out all mandatory fields in previous tabs (Status and Device Info).`):B&&!oe?(q.disabled=!0,q.textContent=`Return device first`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=`This ticket has a device in service. Mark the device as returned (Device Service tab) before completing the service.`):n&&!r?(q.disabled=!0,q.textContent=`Awaiting Payment...`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=`Payment must be detected automatically, or marked paid by admin, before you can submit a resolution.`):(q.disabled=!1,q.textContent=ct?`💰 Save & Resolve`:`Save Changes`,q.style.opacity=`1`,q.style.cursor=`pointer`,q.title=``);else{let e=ye(),t=!0,n=``;e===`status`?(t=s,n=`Please fill out the Work Details / Progress Update first.`):e===`device`&&(t=c,n=`Please fill out the Company Name first.`),t?(q.disabled=!1,q.textContent=`Next →`,q.style.opacity=`1`,q.style.cursor=`pointer`,q.title=``):(q.disabled=!0,q.textContent=`Next →`,q.style.opacity=`0.6`,q.style.cursor=`not-allowed`,q.title=n)}};if(U.onchange=()=>{let e=U.value===`resolved`;Se.style.display=e?`block`:`none`;let t=H.querySelector(`#bill-locked-hint`);t&&(t.style.display=e?`none`:`block`);let n=H.querySelector(`#foc-billno-group`);n&&(n.style.display=U.value===`foc`?`block`:`none`);let r=H.querySelector(`#reschedule-group`);r&&(r.style.display=U.value===`reschedule`?`block`:`none`);let i=U.value===`case_closed`,a=H.querySelector(`#case-closed-hint`);a&&(a.style.display=i?`block`:`none`);let o=H.querySelector(`#progress-detail-label`);o&&(o.innerHTML=i?`Reason for closing (customer didn't cooperate / no fee) <span style="color:var(--danger)">*</span>`:`Work Details / Progress Update <span style="color:var(--danger)">*</span>`),Q()},H.querySelector(`#foc-bill-no`)?.addEventListener(`input`,()=>Q()),H.querySelector(`#reschedule-at`)?.addEventListener(`input`,()=>Q()),Number(j?.reopened)===1&&U.onchange(),ae&&a){let e=H.querySelector(`#device-service-toggle`),t=H.querySelector(`#device-service-body`),r=H.querySelector(`#device-history`),i=H.querySelector(`#followup-body`),o=H.querySelector(`#followup-disabled-hint`),s=H.querySelector(`#followup-history`),c=M?.id||null,l=async()=>{let{data:e}=await b(a),t=e?.device_taken_logs||null,n=e?.device_return_logs||null,i=e?.device_follow_up_logs||[];r&&(r.innerHTML=x(a,t,n,i)),s&&(s.innerHTML=S(i)),oe=!B||!!n||e?.inquiry?.follow_up_status===`returned`||e?.inquiry?.device_status===`returned`||i[0]?.status===`returned`,Q()};e&&(e.onchange=async()=>{let r=e.checked;try{await E(a,r),B=r,t&&(t.style.display=r?`block`:`none`),i&&(i.style.display=r?`block`:`none`),o&&(o.style.display=r?`none`:`block`),n(r?`Device marked for service center`:`Device service turned off`,`success`),r?l():(oe=!0,Q())}catch(t){e.checked=!r,n(t.message||`Could not update`,`error`)}});let u=H.querySelector(`#save-device-taken`);u&&(u.onclick=async()=>{let e=H.querySelector(`#device-taken-image`)?.files?.[0]||null,t=H.querySelector(`#device-taken-desc`)?.value.trim()||``;if(!e)return n(`Please add a photo of the device first`,`warning`);u.disabled=!0,u.textContent=`Saving…`;let{error:r}=await g(a,c,e,t);if(u.disabled=!1,u.textContent=`Save device taken`,r)return n(r.message||`Could not save`,`error`);n(`Device taken saved`,`success`),l()});let d=H.querySelector(`#save-device-followup`);d&&(d.onclick=async()=>{let e=H.querySelector(`#device-followup-status`)?.value,t=H.querySelector(`#device-followup-notes`)?.value.trim()||``;d.disabled=!0,d.textContent=`Saving…`;let{error:r}=await m(a,e,t,c);if(d.disabled=!1,d.textContent=`Add follow-up update`,r)return n(r.message||`Could not save`,`error`);let i=H.querySelector(`#device-followup-notes`);i&&(i.value=``),n(`Follow-up update added`,`success`),l()});let f=H.querySelector(`#save-device-return`);f&&(f.onclick=async()=>{let e=H.querySelector(`#device-return-image`)?.files?.[0]||null,t=H.querySelector(`#device-return-condition`)?.value||`good`,r=H.querySelector(`#device-return-notes`)?.value.trim()||``;if(!e)return n(`Please add a photo of the returned device first`,`warning`);f.disabled=!0,f.textContent=`Saving…`;let{error:i}=await h(a,e,t,r);if(f.disabled=!1,f.textContent=`Mark returned / sent back to client`,i)return n(i.message||`Could not save`,`error`);n(`Device return recorded`,`success`),l()}),B&&l()}W.oninput=()=>{X(),Q()},Te&&(Te.oninput=()=>{X(),Q()}),Ee&&(Ee.onchange=()=>{X(),Q()}),De&&(De.oninput=()=>{X(),Q()}),Oe&&(Oe.oninput=()=>{X(),Q()});let lt=!1,ut=()=>{lt||U.value===`resolved`&&(xe(`bill`),Q(),setTimeout(()=>{let e=H.querySelector(`#save-update`);e&&!e.disabled&&(lt=!0,e.click())},300))},dt=async e=>{if(!a)return;let t=null;try{let e=localStorage.getItem(`auth_token`)||(await f.auth.getSession()).data.session?.access_token,n=await fetch(`/api/payments/check-status`,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${e}`},body:JSON.stringify({inquiry_id:a})});n.ok&&(t=await n.json())}catch{}if(t||=(await f.from(`inquiries`).select(`payment_status,payment_received_at`).eq(`id`,a).single()).data,!t)return;let i=A.status===`paid`;if(A={status:t.payment_status||`unpaid`,received_at:t.payment_received_at||null},!i&&A.status===`paid`){ct=!0,r({title:`💰 Payment Received`,body:`Saving the resolution…`,type:`payment`,tag:`pay-${a}`}),Q(),ut();return}else e&&n(A.status===`paid`?`Payment confirmed`:`Still waiting for client to pay...`,A.status===`paid`?`success`:`info`);Q()};H.querySelector(`#emp-pay-check`).onclick=()=>dt(!0);let ft=null,pt=()=>{ft&&=(clearInterval(ft),null)};A.status!==`paid`&&(ft||!a||(ft=setInterval(async()=>{if(A.status===`paid`)return pt();await dt(!1)},3e3)));let mt=f.channel(`task-modal-${a||e}`).on(`postgres_changes`,{event:`UPDATE`,schema:`public`,table:`inquiries`,filter:a?`id=eq.${a}`:``},e=>{let t=e.new;if(!t||a&&t.id!==a)return;let n=A.status===`paid`;if(t.payment_status&&(A.status=t.payment_status),t.payment_received_at&&(A.received_at=t.payment_received_at),!n&&A.status===`paid`){ct=!0,r({title:`💰 Payment Received!`,body:`Client just paid — saving the resolution…`,type:`payment`,tag:`pay-${a}`}),Q(),ut();return}Q()}).subscribe();Ze(),X(),Q(),z&&(H.querySelector(`#progress-detail`).disabled=!0,H.querySelectorAll(`
        ...
        #emp-gen-link,
        #emp-pay-check
      `).forEach(e=>{e.disabled=!0,e.style.opacity=`0.6`,e.style.cursor=`not-allowed`}),H.querySelectorAll(`.mst-pane[data-pane="device"] input, .mst-pane[data-pane="device"] select`).forEach(e=>{e.disabled=!0,e.style.opacity=`0.6`,e.style.cursor=`not-allowed`}));let ht=H.querySelector(`#emp-gen-link`),gt=H.querySelector(`#emp-pay-link`),_t=H.querySelector(`#emp-qr-wrap`),vt=H.querySelector(`#emp-qr-img`),yt=H.querySelector(`#emp-share-wa`),bt=e=>{Pe=e,gt.value=e,vt.src=`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(e)}`,_t.style.display=`block`,yt.style.display=`flex`};ht&&(ht.onclick=async()=>{let e=Number(Ce.value)||0;if(!e){n(`Select services or enter a bill amount first`,`warning`);return}if(!Le())return;let{data:t}=await f.from(`inquiries`).select(`full_name,phone,service_item,ticket_no`).eq(`id`,a).single();if(!t){n(`Could not load inquiry details`,`error`);return}ht.disabled=!0,ht.textContent=`...`;try{let r=Ae();await Me(r);let i=localStorage.getItem(`auth_token`)||(await f.auth.getSession()).data.session?.access_token,o=await fetch(`/api/payments/create-link`,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${i}`},body:JSON.stringify({amount:e,description:`Service: ${t.service_item}`,ticket_no:t.ticket_no,customer:{name:t.full_name,phone:t.phone}})}),s=await o.json();if(!o.ok)throw Error(s.error||`Failed`);bt(s.short_url),await f.from(`inquiries`).update({payment_link:s.short_url,payment_link_id:s.id||null,bill_amount:Y.servicesSubtotal+Y.extra,transport_km:Y.km,transport_fee:Y.transport,platform_fee:Y.platform,discount_amount:Y.discount,discount_label:Y.discountLabel||null,discount_reason:Y.manualDiscount>0?Y.discountReason:null,discount_preset_id:Y.discountPresetId||null,gst_amount:Y.gst,bill_total:Y.total,bill_generated_at:new Date().toISOString().slice(0,19).replace(`T`,` `),device_type:H.querySelector(`#device-type`)?.value.trim()||null,device_serial_no:H.querySelector(`#device-serial`)?.value.trim()||null,company_name:r||null,employee_bill_lat:Z.lat,employee_bill_lng:Z.lng}).eq(`id`,a),st=!0,Q(),n(`Payment link generated! Waiting for client to pay...`,`success`)}catch(e){n(e.message,`error`)}finally{ht.disabled=!1,ht.innerHTML=`${u.link}<span>Generate</span>`}});let xt=H.querySelector(`#open-bill-modal`);xt&&(xt.onclick=async()=>{if(Y.total<=0){n(`Add services or charges first to generate a bill`,`warning`);return}if(!Le())return;let e=xt.innerHTML;xt.disabled=!0,xt.innerHTML=`<span class="btn-spinner"></span><span>Saving PDF...</span>`;try{let e=Ae();await Me(e);let{data:t}=a?await f.from(`inquiries`).select(`*`).eq(`id`,a).single():{data:j},r=Ie(t||j||{}),i=await ce(r,{inquiryId:a||null});if(Ne=i,a){let t={bill_amount:Y.servicesSubtotal+Y.extra,transport_km:Y.km,transport_fee:Y.transport,platform_fee:Y.platform,discount_amount:Y.discount,discount_label:Y.discountLabel||null,discount_reason:Y.manualDiscount>0?Y.discountReason:null,discount_preset_id:Y.discountPresetId||null,gst_amount:Y.gst,bill_total:Y.total,bill_generated_at:new Date().toISOString().slice(0,19).replace(`T`,` `),bill_pdf_url:i,device_type:r.customer.device_type||null,device_serial_no:r.customer.device_serial||null,company_name:e||null,employee_bill_lat:Z.lat,employee_bill_lng:Z.lng},{error:n}=await f.from(`inquiries`).update(t).eq(`id`,a);if(n)throw Error(n.message);j={...j||{},...t}}Fe(),n(`Bill PDF saved. You can download or send it from this tab.`,`success`),ue(r,{inquiryId:a||null,existingPdfUrl:i,onSent:async e=>{e&&(Ne=e,Fe())}})}catch(e){console.error(e),n(e.message||`Could not save bill PDF`,`error`)}finally{xt.disabled=!1,xt.innerHTML=e}});let St=H.querySelector(`#bill-details-whatsapp`);St&&(St.onclick=()=>{let e=Ie(j||{}),t=(e.customer?.phone||``).replace(/\D/g,``);if(!t){n(`Client phone number is missing on this inquiry`,`error`);return}let r=`https://wa.me/${t}?text=${encodeURIComponent(le(e,null))}`;window.open(r,`_blank`)}),Fe(),yt&&(yt.onclick=()=>{Pe&&window.open(`https://wa.me/?text=${encodeURIComponent(`Please use this link to pay for your service: `+Pe)}`,`_blank`)});let Ct=H.querySelector(`#pay-method-toggle`),wt=H.querySelector(`#cash-section`),Tt=H.querySelector(`#emp-pay-link`)?.closest(`div[style*="background:var(--bg-soft)"]`),Et=H.querySelector(`#cash-amount-display`),Dt=H.querySelector(`#cash-collected-banner`),$=H.querySelector(`#mark-cash-btn`),Ot=j?.payment_method===`cash`?`cash`:`online`,kt=()=>{Ct.querySelectorAll(`.pay-method-btn`).forEach(e=>{e.classList.toggle(`active`,e.dataset.method===Ot)}),wt.style.display=Ot===`cash`?`block`:`none`,Tt&&(Tt.style.display=Ot===`cash`?`none`:`block`),Et&&(Et.textContent=`₹${Math.round(Y.total).toLocaleString(`en-IN`)}`),j?.payment_method===`cash`&&j?.payment_status===`paid`&&Dt&&(Dt.style.display=`block`,$&&($.disabled=!0,$.style.opacity=`0.55`))};Ct.querySelectorAll(`.pay-method-btn`).forEach(e=>{e.onclick=()=>{Ot=e.dataset.method,kt()}}),kt(),$&&($.onclick=async()=>{if(!a){n(`Cannot mark cash on a task without an inquiry record`,`error`);return}if(Y.total<=0){n(`Add services or charges first`,`warning`);return}if(!Le())return;let e=Ae();$.disabled=!0;let t=$.innerHTML;$.innerHTML=`<span>Saving...</span>`;try{await Me(e);let t=new Date().toISOString().slice(0,19).replace(`T`,` `),r={payment_method:`cash`,payment_status:`paid`,payment_received_at:t,cash_collected_at:t,bill_amount:Y.servicesSubtotal+Y.extra,extra_cost:Y.extra,extra_cost_reason:H.querySelector(`#extra-reason`)?.value.trim()||null,transport_km:Y.km,transport_fee:Y.transport,platform_fee:Y.platform,discount_amount:Y.discount,discount_label:Y.discountLabel||null,discount_reason:Y.manualDiscount>0?Y.discountReason:null,discount_preset_id:Y.discountPresetId||null,gst_amount:Y.gst,bill_total:Y.total,bill_generated_at:t,device_type:H.querySelector(`#device-type`)?.value.trim()||null,device_serial_no:H.querySelector(`#device-serial`)?.value.trim()||null,company_name:e||null,employee_bill_lat:Z.lat,employee_bill_lng:Z.lng},{error:i}=await f.from(`inquiries`).update(r).eq(`id`,a);if(i)throw Error(i.message);A={status:`paid`,received_at:t},j={...j||{},...r},ct=!0,Q(),kt(),n(`✓ Cash recorded — finalising the ticket…`,`success`),ut()}catch(e){n(e.message||`Could not mark cash`,`error`),$.disabled=!1,$.innerHTML=t}});let At=()=>{pt();try{f.removeChannel(mt)}catch{}H.remove()},jt=H.querySelector(`#cm`);jt&&(jt.onclick=At);let Mt=H.querySelector(`#cm2`);Mt&&(Mt.onclick=At),H.addEventListener(`click`,e=>{e.target===H&&At()}),H.querySelector(`#save-update`).onclick=async()=>{let t=U.value,r=t===`resolved`;if(r&&!be()){xe(ve[ve.indexOf(ye())+1]);return}if(z){n(`Resolved tasks are read-only`,`info`);return}let i=H.querySelector(`#progress-detail`).value.trim(),o=Ae(),s=t===`foc`,c=H.querySelector(`#foc-bill-no`)?.value.trim()||``,l=t===`reschedule`,u=H.querySelector(`#reschedule-at`)?.value||``,p=l?`in_progress`:t;if(l&&!u){n(`Pick the new visit date & time`,`warning`);return}if(!l&&!i){n(`Please provide details of your work`,`warning`);return}if(s){if(!c){n(`Enter the client's bill number for this FOC service`,`warning`);return}if(B&&!oe){n(`Mark the device as returned before closing this service`,`warning`);return}}if(r){if(!o){n(`Please provide the company name`,`warning`);return}if(!Le())return}let m=H.querySelector(`#save-update`);m.disabled=!0,m.textContent=`Saving...`;try{r&&o&&await Me(o)}catch(e){console.error(`Failed to ensure company:`,e)}let h=[];r&&V.forEach(e=>{h.push(e.id)});let{data:{user:g}}=await f.auth.getUser(),_=[];e&&_.push(f.from(`tickets`).update({status:p}).eq(`id`,e));let v={status:p,employee_update_detail:i,employee_update_status:t,employee_update_at:new Date().toISOString().slice(0,19).replace(`T`,` `)},y=H.querySelector(`#device-type`)?.value.trim(),b=H.querySelector(`#device-serial`)?.value.trim();o&&(v.company_name=o),y&&(v.device_type=y),b&&(v.device_serial_no=b),l&&(v.scheduled_at=u?new Date(u).toISOString().slice(0,19).replace(`T`,` `):null),s&&(v.bill_no=c,v.bill_total=0,v.bill_amount=0,v.payment_status=`foc`),r&&Y.total>0&&(v.bill_amount=Y.servicesSubtotal+Y.extra,v.extra_cost=Y.extra,v.extra_cost_reason=H.querySelector(`#extra-reason`).value.trim()||null,v.transport_km=Y.km,v.transport_fee=Y.transport,v.platform_fee=Y.platform,v.discount_amount=Y.discount,v.discount_label=Y.discountLabel||null,v.discount_reason=Y.manualDiscount>0?Y.discountReason:null,v.discount_preset_id=Y.discountPresetId||null,v.gst_amount=Y.gst,v.bill_total=Y.total,v.employee_bill_lat=Z.lat,v.employee_bill_lng=Z.lng),a?_.push(f.from(`inquiries`).update(v).eq(`id`,a)):e&&_.push(f.from(`inquiries`).update(v).eq(`ticket_id`,e)),a&&(await f.from(`inquiry_services`).delete().eq(`inquiry_id`,a),h.length>0&&_.push(f.from(`inquiry_services`).insert(h.map(e=>({inquiry_id:a,service_id:e}))))),e&&_.push(f.from(`ticket_comments`).insert({ticket_id:e,user_id:g.id,content:`[Status: ${t.replace(`_`,` `)}] ${i}${r&&Y.total>0?` (Bill: ₹${Y.total})`:``}`})),await Promise.all(_),n(`Task updated!`,`success`),t===`resolved`&&n(`Secure feedback link will be sent after payment is received`,`info`),At(),d()}})()}function Ie(e,t){let r=document.createElement(`div`);r.className=`modal-overlay`,r.innerHTML=`
    <div class="modal" style="max-width:450px">
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-primary" id="btn-submit-leave">Submit Request</button>
          ...
          <tr data-main="${K(e.category||`Service`)}" data-sub="${K(e.sub_category||``)}" data-search="${K(`${e.category||``} ${e.sub_category||``} ${e.sub_sub_category||``} ${e.name||``}`.toLowerCase())}">
            <td><input type="checkbox" class="service-checkbox" data-id="${e.id}"></td>
            <td><span class="badge badge-open">${G(e.category||`Service`)}</span></td>
            <td>${e.sub_category?G(e.sub_category):`<span style="color:var(--text-dim)">-</span>`}</td>
            <td><b>${G(e.sub_sub_category||e.name||``)}</b></td>
            <td>${Ce(e.cost)}</td>
        ...
        <div class="card-body" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;align-items:end;">
          ...
          <div class="form-group" style="margin:0;">
            ...
            <select id="emp-price-sub">
              ...
              ${s.map(e=>`<option value="${K(e)}">${G(e)}</option>`).join(``)}
            </select>
          </div>
          <div class="form-group" style="margin:0;">
            <label>Search</label>
