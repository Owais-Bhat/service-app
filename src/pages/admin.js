...
export async function renderAdminDashboard(container) {
  ...
  const onTimeToday = allRows.filter((x) => {
    ...
    const deadline = calculateSLA(x.assigned_at || x.created_at);
...
export async function openInquiryDetail(id, onDone) {
  ...
  const overlay = document.createElement("div");
  overlay.className = "modal-overlay";
  const serviceDeadline = i.assigned_at ? calculateSLA(i.assigned_at) : null;
...
export async function renderAttendance(container) {
  ...
  container.querySelector("#att-ok").onclick = () => { doFilter(); attClose(); };
...
function openLocationMapModal(lat, lng, label) {
  const overlay = document.createElement("div");
  overlay.className = "modal-overlay";
  overlay.innerHTML = `    <div class="modal" style="max-width:420px">      <div class="modal-header">        <span class="modal-title">${escapeHtml(label || "Location")}</span>        <button class="modal-close" id="loc-map-close">${ICONS.close || "×"}</button>      </div>      <div class="modal-body" style="padding:0;">        <div id="loc-map-el" style="height:260px;width:100%;"></div>        <div style="padding:10px 16px;display:flex;justify-content:space-between;align-items:center;">          <small style="color:var(--text-dim);">${lat.toFixed(6)}, ${lng.toFixed(6)}</small>          <a href="${mapLink(lat, lng)}" target="_blank" rel="noopener" style="color:var(--primary);font-size:0.82rem;text-decoration:none;">Open in Google Maps</a>        </div>      </div>    </div>  `;
  document.body.appendChild(overlay);
  const close = () => overlay.remove();
  overlay.querySelector("#loc-map-close").onclick = close;
  overlay.onclick = (e) => {
  ...
  if (typeof L !== "undefined") {
    ...
    overlay.querySelector("#loc-map-el").innerHTML =
...
export async function renderEODReports(container) {
  ...
  const render = (items) => {
    ...
    container.querySelector("#eod-ok").onclick = () => { doFilter(); eodClose(); };
...
export async function renderUsers(container) {
          ...
          <tbody>
            ${
              ...
              <tr class="user-row" data-uid="${u.id}" style="cursor:pointer;">
                ...
                <td><span class="badge ${u.role === "admin" ? "badge-danger" : u.role === "employee" ? "badge-open" : "badge-resolved"}">${escapeHtml(u.role || "client")}</span>${u.role === "employee" ? `<span class="badge ${u.worker_type === "gig" ? "badge-assigned" : "badge-in_progress"}" style="margin-left:4px;">${u.worker_type === "gig" ? "Gig" : "Fixed"}</span>` : ""}</td>
                <td>
  ...
  function openUserModal(user, onDone) {
    ...
    const EMP_TABS = [
      ...
      { id: "my-installations", label: "Installations" },
...
export async function renderCashCollectionsTab(container) {
  ...
  container.innerHTML = `    <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;">      <div>        <h1>Cash Collections</h1>        <p>Cash collected by technicians, grouped per employee. Tick the records you're receiving cash for, then press <b>Record Submission</b>.</p>      </div>    </div>    <div class="stats-grid" style="margin-bottom:24px;">      <div class="stat-card">        <div class="stat-value" style="color:var(--warning); font-size:1.9rem;">\u20B9${Math.round(totalPending).toLocaleString("en-IN")}</div>        <div class="stat-label">Total Pending</div>      </div>      <div class="stat-card">        <div class="stat-value" style="color:var(--success); font-size:1.9rem;">\u20B9${Math.round(totalSubmitted).toLocaleString("en-IN")}</div>        <div class="stat-label">Total Submitted (All Time)</div>      </div>      <div class="stat-card">        <div class="stat-value">${empsWithCash.length}</div>        <div class="stat-label">Employees With Records</div>      </div>      <div class="stat-card">        <div class="stat-value">${allPending.length}</div>        <div class="stat-label">Pending Records</div>      </div>    </div>    ${empsWithCash.length === 0 ? '<div class="card"><div class="card-body" style="text-align:center; padding:48px; color:var(--text-dim);">No cash collections recorded yet.</div></div>' : empsWithCash.map(empCardHtml).join("")}  `;
...
function openAdEditor(ad, onChange) {
  ...
  overlay.querySelectorAll('input[name="ad-kind"]').forEach((r) => {
    r.addEventListener('change', () => {
      const audioDiv = overlay.querySelector('#ad-audio-container');
      if (audioDiv) audioDiv.style.display = r.value === 'video' ? 'flex' : 'none';
    });
  });
  overlay.querySelector("#ad-save").onclick = async () => {
    ...
    const audio_enabled = overlay.querySelector("#ad-audio-enabled").checked ? 1 : 0;
