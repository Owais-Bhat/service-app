...
export async function renderAdminDashboard(container) {
  ...
  bind("#admin-enable-alerts", async () => {
    ...
    showNotification({
      ...
      body:
        permission === "granted"
          ? "Live admin alerts will show with sound."
          : "In-app alerts will show with sound after browser interaction.",
      type: "success",
...
export async function renderAllTickets(container) {
  ...
  const ticketRows = (tickets || [])
    .map((t) => {
      ...
      return `<tr>      <td><code style="font-size:0.75rem;">#${t.id.slice(0, 8)}</code></td>      <td><b>${inq ? inq.full_name : "Guest"}</b></td>      <td>${contact}</td>      <td>${t.assigned_to ? profileMap[t.assigned_to] || "Staff" : '<span style="color:var(--text-dim)">Unassigned</span>'}</td>      <td>${statusBadge(t.status)}</td>      <td><small>${formatDate(t.created_at)}</small></td>      <td>${action}</td>    </tr>`;
    })
    .join("");
        ...
        <table>
          ...
          <tbody>
            ...
            `,
                    ...
                    .join("")
                : '<tr><td colspan="7" style="text-align:center;padding:32px;color:var(--text-dim)">No users found</td></tr>'
            }
          </tbody>
        </table>
      </div>
