...
function assignedGroupOf(r) {
  ...
  // Waiting on money means money is actually due: a bill was generated or a
  // payment link went out, and it hasn't been settled. A plain progress note
  // from the employee doesn't count — that job is still running.
  const billed = Number(r.bill_total) > 0 || Number(r.bill_amount) > 0;
  if ((billed || r.payment_link) && !settled) return 'payment';
  return 'ongoing';
}
